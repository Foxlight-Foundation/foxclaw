...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["foxclaw"]
        let text = "hey foxclaw"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey foxclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["foxclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "foxclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "foxclaw")),
