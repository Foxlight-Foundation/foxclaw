...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: FoxClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FoxClawProtocol.AnyCodable] = [
            "presence": .init([FoxClawProtocol.AnyCodable(entry)]),
        ]
        let payload = FoxClawProtocol.AnyCodable(payloadMap)
