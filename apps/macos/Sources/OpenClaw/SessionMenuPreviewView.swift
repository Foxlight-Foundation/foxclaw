...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.foxclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FoxClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FoxClawSessionPreviewEntry,
    ...
    private static func cache(payload: FoxClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FoxClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FoxClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FoxClawKit.AnyCodable]) -> [FoxClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FoxClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FoxClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FoxClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FoxClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FoxClawChatMessage) -> String? {
