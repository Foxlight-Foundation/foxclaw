...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.foxclaw.mac`) even if the shipped
    ...
    {
        if environment["FOXCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "foxclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "foxclaw.nixMode") { return true }
