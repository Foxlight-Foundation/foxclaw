...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FoxClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.foxclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FoxClawKit.AnyCodable]()) {
                $0[$1.key] = FoxClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FoxClawProtocol.AnyCodable?) -> [String: FoxClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FoxClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FoxClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FoxClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("foxclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("foxclaw.control.agent")
}
