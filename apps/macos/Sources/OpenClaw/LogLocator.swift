...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FOXCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/foxclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("foxclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("foxclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/foxclaw/ (rolling or stdout), or nil if none exist.
