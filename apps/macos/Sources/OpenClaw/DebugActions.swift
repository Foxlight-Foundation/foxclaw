...
enum DebugActions {
    private static let verboseDefaultsKey = "foxclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "foxclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FoxClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FoxClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FoxClawPaths.configURL
        guard
