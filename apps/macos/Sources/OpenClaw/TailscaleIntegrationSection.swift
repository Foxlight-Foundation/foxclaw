...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.foxclaw/foxclaw.json. Prefer FOXCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.foxclaw/foxclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.foxclaw/foxclaw.json. Restart the gateway to apply."
        }
