...
struct FoxClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("foxclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      foxclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      foxclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      foxclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      foxclaw-mac connect
      foxclaw-mac connect --url ws://127.0.0.1:18789 --json
      foxclaw-mac discover --timeout 3000 --json
      foxclaw-mac wizard --mode local
    """)
}
