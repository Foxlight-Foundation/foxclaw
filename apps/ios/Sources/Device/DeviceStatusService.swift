...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> FoxClawDeviceStatusPayload {
        ...
        return FoxClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> FoxClawDeviceInfoPayload {
        ...
        return FoxClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> FoxClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: FoxClawBatteryState = switch device.batteryState {
        ...
        return FoxClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> FoxClawThermalStatusPayload {
        let state: FoxClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return FoxClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> FoxClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return FoxClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
