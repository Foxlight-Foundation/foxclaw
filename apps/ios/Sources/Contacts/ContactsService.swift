...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FoxClawContactsSearchParams) async throws -> FoxClawContactsSearchPayload {
        ...
        return FoxClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FoxClawContactsAddParams) async throws -> FoxClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FoxClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FoxClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FoxClawContactPayload {
        FoxClawContactPayload(
