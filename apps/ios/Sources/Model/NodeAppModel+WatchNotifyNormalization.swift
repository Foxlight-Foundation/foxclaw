...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: FoxClawWatchNotifyParams) -> FoxClawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [FoxClawWatchAction]?,
        kind: String?,
        promptId: String?) -> [FoxClawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> FoxClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FoxClawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                FoxClawWatchAction(id: "approve", label: "Approve"),
                FoxClawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                FoxClawWatchAction(id: "open_phone", label: "Open iPhone"),
                FoxClawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            FoxClawWatchAction(id: "done", label: "Done"),
            FoxClawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            FoxClawWatchAction(id: "open_phone", label: "Open iPhone"),
            FoxClawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: FoxClawWatchRisk?,
        priority: FoxClawNotificationPriority?) -> FoxClawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: FoxClawNotificationPriority?,
        risk: FoxClawWatchRisk?) -> FoxClawNotificationPriority?
    {
