...
final class RemindersService: RemindersServicing {
    func list(params: FoxClawRemindersListParams) async throws -> FoxClawRemindersListPayload {
        ...
        let payload: [FoxClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return FoxClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FoxClawRemindersAddParams) async throws -> FoxClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FoxClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FoxClawRemindersAddPayload(reminder: payload)
    }
