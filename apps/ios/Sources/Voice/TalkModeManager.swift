...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__FOXCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<FoxClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.foxclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = FoxClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> FoxClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return FoxClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> FoxClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> FoxClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return FoxClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> FoxClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = FoxClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: FoxClawTalkPTTStopPayload) {
