...
final class MotionService: MotionServicing {
    func activities(params: FoxClawMotionActivityParams) async throws -> FoxClawMotionActivityPayload {
        ...
        let mapped: [FoxClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return FoxClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FoxClawPedometerParams) async throws -> FoxClawPedometerPayload {
        ...
        let payload: FoxClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FoxClawPedometerPayload(
