...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.foxclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to FoxClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to FoxClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to FoxClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "FoxClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "FoxClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "FoxClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("foxclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
