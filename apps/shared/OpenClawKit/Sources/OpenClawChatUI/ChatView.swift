...
@MainActor
public struct FoxClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: FoxClawChatViewModel
    ...
    public init(
        viewModel: FoxClawChatViewModel,
    ...
    private var visibleMessages: [FoxClawChatMessage] {
        let base: [FoxClawChatMessage]
    ...
    private func mergeToolResults(in messages: [FoxClawChatMessage]) -> [FoxClawChatMessage] {
    ...
    private func isToolResultMessage(_ message: FoxClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: FoxClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: FoxClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: FoxClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: FoxClawChatMessage) -> [FoxClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: FoxClawChatMessage) -> [FoxClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: FoxClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: FoxClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(FoxClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
