...
private let chatUILogger = Logger(subsystem: "ai.foxclaw", category: "FoxClawChatUI")

@MainActor
@Observable
public final class FoxClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [FoxClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [FoxClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [FoxClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FoxClawChatPendingToolCall] = []
    public private(set) var sessions: [FoxClawChatSessionEntry] = []
    private let transport: any FoxClawChatTransport
    private var sessionDefaults: FoxClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: FoxClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any FoxClawChatTransport,
    ...
    public var sessionChoices: [FoxClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [FoxClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FoxClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FoxClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FoxClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: FoxClawChatMessage) -> FoxClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> FoxClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return FoxClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: FoxClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [FoxClawChatMessage],
        incoming: [FoxClawChatMessage]) -> [FoxClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [FoxClawChatMessage]) -> [FoxClawChatMessage] {
        var result: [FoxClawChatMessage] = []
    ...
    private static func dedupeKey(for message: FoxClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FoxClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> FoxClawChatAttachmentPayload in
            FoxClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FoxClawChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = FoxClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: FoxClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FoxClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FoxClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FoxClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FoxClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
