...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> FoxClawChatHistoryPayload
{
    FoxClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> FoxClawChatSessionEntry {
    FoxClawChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> FoxClawChatSessionEntry
{
    FoxClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> FoxClawChatModelChoice {
    FoxClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [FoxClawChatHistoryPayload],
    sessionsResponses: [FoxClawChatSessionsListResponse] = [],
    modelResponses: [[FoxClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, FoxClawChatViewModel)
{
...
private func sendUserMessage(_ vm: FoxClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, FoxClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FoxClawChatHistoryPayload]
    private let sessionsResponses: [FoxClawChatSessionsListResponse]
    private let modelResponses: [[FoxClawChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<FoxClawChatTransportEvent>
    private let continuation: AsyncStream<FoxClawChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<FoxClawChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<FoxClawChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> FoxClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FoxClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [FoxClawChatAttachmentPayload]) async throws -> FoxClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return FoxClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FoxClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FoxClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [FoxClawChatModelChoice] {
    ...
    func emit(_ evt: FoxClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = FoxClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = FoxClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: FoxClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = FoxClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: FoxClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(FoxClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == FoxClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = FoxClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: FoxClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = FoxClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = FoxClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = FoxClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { FoxClawChatViewModel(sessionKey: "main", transport: transport) }
