...
val androidStoreFile = providers.gradleProperty("FOXCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("FOXCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("FOXCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("FOXCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set FOXCLAW_ANDROID_STORE_FILE, " +
            "FOXCLAW_ANDROID_STORE_PASSWORD, FOXCLAW_ANDROID_KEY_ALIAS, and " +
            "FOXCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/FoxClawKit/Sources/FoxClawKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.foxclaw.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "foxclaw-$versionName-$buildType.apk"
