...
This document describes how the FoxClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`foxclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
FoxClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing FoxClaw Gateway
...
- ACP client spawns `foxclaw acp` and speaks ACP messages over stdio.
...
`foxclaw acp` resolves the Gateway URL and auth from CLI flags or config:
