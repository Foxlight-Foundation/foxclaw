...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.FOXCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    FOXCLAW_PROFILE: profile,
    FOXCLAW_GATEWAY_PORT: String(port),
    FOXCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    FOXCLAW_SYSTEMD_UNIT: systemdUnit,
    FOXCLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    FOXCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    FOXCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.FOXCLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    FOXCLAW_GATEWAY_TOKEN: gatewayToken,
    FOXCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    FOXCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    FOXCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    FOXCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    FOXCLAW_LOG_PREFIX: "node",
    FOXCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    FOXCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    FOXCLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    FOXCLAW_STATE_DIR: sharedEnv.stateDir,
    FOXCLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.FOXCLAW_STATE_DIR;
  const configPath = env.FOXCLAW_CONFIG_PATH;
