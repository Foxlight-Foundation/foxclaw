...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds FOXCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `foxclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service FOXCLAW_GATEWAY_TOKEN does not match gateway.auth.token in foxclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.FOXCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.FOXCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `foxclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
