...
import {
  probeAuthenticatedFoxClawRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedFoxClawRelay({
    baseUrl,
    relayAuthHeader: "x-foxclaw-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.FOXCLAW_GATEWAY_TOKEN;
  ...
  it("accepts authenticated foxclaw relay probe responses", async () => {
