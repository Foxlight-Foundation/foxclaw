...
const { stopFoxClawChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopFoxClawChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopFoxClawChrome: stopFoxClawChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "foxclaw", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopFoxClawChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["foxclaw", "chrome-relay"]);
    ...
    expect(stopMap.foxclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopFoxClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("foxclaw browser stop failed: Error: oops");
  });
});
