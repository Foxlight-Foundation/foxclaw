...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".foxclaw-profile-decorated");
}
 ...
 */
export function decorateFoxClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FOXCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FOXCLAW_BROWSER_COLOR).toUpperCase();
