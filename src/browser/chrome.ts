...
export function resolveFoxClawUserDataDir(profileName = DEFAULT_FOXCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchFoxClawChrome(
  ...
  const userDataDir = resolveFoxClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_FOXCLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateFoxClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`foxclaw browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopFoxClawChrome(
