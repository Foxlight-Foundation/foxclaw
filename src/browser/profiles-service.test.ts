...
vi.mock("./chrome.js", () => ({
  resolveFoxClawUserDataDir: vi.fn(() => "/tmp/foxclaw-test/foxclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "foxclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveFoxClawUserDataDir).mockReturnValue(userDataDir);
