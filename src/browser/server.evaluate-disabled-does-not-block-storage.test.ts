  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          foxclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.FOXCLAW_GATEWAY_PORT;
    process.env.FOXCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.FOXCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.FOXCLAW_GATEWAY_PASSWORD;
