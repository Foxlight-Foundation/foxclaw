...
function localFoxClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "foxclaw",
...
function createLocalFoxClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localFoxClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveFoxClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foxclaw-reset-"));
    const profileDir = path.join(tempRoot, "foxclaw");
    ...
    const ops = createLocalFoxClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveFoxClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foxclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "foxclaw");
    fs.mkdirSync(profileDir, { recursive: true });
