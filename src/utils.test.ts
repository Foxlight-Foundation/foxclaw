...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("foxclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("foxclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("foxclaw-lid-a-", async (first) => {
      await withTempDir("foxclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.foxclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "foxclaw-config-dir-"));
    try {
  ...
  it("expands FOXCLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/foxclaw-home",
      FOXCLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/foxclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers FOXCLAW_HOME over HOME", () => {
    vi.stubEnv("FOXCLAW_HOME", "/srv/foxclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/foxclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $FOXCLAW_HOME prefix when FOXCLAW_HOME is set", () => {
    vi.stubEnv("FOXCLAW_HOME", "/srv/foxclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/foxclaw-home")}/.foxclaw/foxclaw.json`)).toBe(
      "$FOXCLAW_HOME/.foxclaw/foxclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FOXCLAW_HOME replacement when FOXCLAW_HOME is set", () => {
    vi.stubEnv("FOXCLAW_HOME", "/srv/foxclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/foxclaw-home")}/.foxclaw/foxclaw.json`),
    ).toBe("config: $FOXCLAW_HOME/.foxclaw/foxclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/foxclaw", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "foxclaw"),
    );
  });
  ...
  it("prefers FOXCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("FOXCLAW_HOME", "/srv/foxclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/foxclaw")).toBe(path.resolve("/srv/foxclaw-home", "foxclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/foxclaw", env)).toBe(path.resolve("/srv/foxclaw-home", "foxclaw"));
  });
