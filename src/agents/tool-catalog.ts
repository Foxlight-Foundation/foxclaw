...
function buildCoreToolGroupMap() {
  ...
  const foxclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFoxClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:foxclaw": foxclawTools,
