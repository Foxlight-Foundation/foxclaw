...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; FoxClaw handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "For FoxClaw behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `foxclaw status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running FoxClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List FoxClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List FoxClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside FoxClaw.";
  }

  const lines = [
    "You are a personal assistant running inside FoxClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control FoxClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## FoxClaw CLI Quick Reference",
    "FoxClaw is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- foxclaw gateway status",
    "- foxclaw gateway start",
    "- foxclaw gateway stop",
    "- foxclaw gateway restart",
    "If unsure, ask the user to run `foxclaw help` (or `foxclaw gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## FoxClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, FoxClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by FoxClaw and included below in Project Context.",
    "",
