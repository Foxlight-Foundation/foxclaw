...
import { markFoxClawExecEnv } from "../../infra/foxclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "foxclaw.sandbox=1");
  args.push("--label", `foxclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `foxclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `foxclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markFoxClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "foxclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`foxclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`foxclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("foxclaw sandbox recreate --all");
}
