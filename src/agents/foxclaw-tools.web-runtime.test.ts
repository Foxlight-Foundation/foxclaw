...
function findTool(name: string, config: FoxClawConfig) {
  const allTools = createFoxClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: FoxClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("foxclaw tools runtime web metadata wiring", () => {
