...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const foxClawTmpDir = path.resolve(resolvePreferredFoxClawTmpDir());
  if (!isPathInside(foxClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: foxClawTmpDir });
  return resolved;
}
