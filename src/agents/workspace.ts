...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.FOXCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".foxclaw", `workspace-${profile}`);
  }
  return path.join(home, ".foxclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".foxclaw";
