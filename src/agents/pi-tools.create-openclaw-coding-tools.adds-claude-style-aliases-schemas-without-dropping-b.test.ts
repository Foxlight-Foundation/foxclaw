...
const defaultTools = createFoxClawCodingTools({ senderIsOwner: true });
...
describe("createFoxClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: FoxClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createFoxClawCodingTools({
    ...
    const anthropicTools = createFoxClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: FoxClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createFoxClawCodingTools({
    ...
    const denied = createFoxClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createFoxClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
