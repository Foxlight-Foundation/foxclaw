...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): FoxClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureFoxClawModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureFoxClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
