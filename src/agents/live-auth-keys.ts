...
const GOOGLE_LIVE_SINGLE_KEY = "FOXCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "FOXCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "FOXCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "FOXCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `FOXCLAW_LIVE_${base}_KEY`;
