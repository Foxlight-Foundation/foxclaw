...
export function isCliProvider(provider: string, cfg?: FoxClawConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: FoxClawConfig; agentId?: string }): string[] {
  if (params.agentId) {
