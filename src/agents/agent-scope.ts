...
type AgentEntry = NonNullable<NonNullable<FoxClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: FoxClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: FoxClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: FoxClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: FoxClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: FoxClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: FoxClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: FoxClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
