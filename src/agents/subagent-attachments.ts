...
function resolveAttachmentLimits(config: FoxClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".foxclaw", "attachments");
  const relDir = path.posix.join(".foxclaw", "attachments", attachmentId);
