...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    foxclaw: process.env.FOXCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.foxclaw === undefined) {
      delete process.env.FOXCLAW_GATEWAY_TOKEN;
    } else {
      process.env.FOXCLAW_GATEWAY_TOKEN = envSnapshot.foxclaw;
    }
  ...
  it("uses FOXCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.FOXCLAW_GATEWAY_TOKEN = "local-env-token";
