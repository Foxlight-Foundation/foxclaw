...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "foxclaw" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [{ title: "FoxClaw", url: "https://foxclaw.ai/docs", content: "docs" }],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest foxclaw release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://foxclaw.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://foxclaw.ai/docs"]);
