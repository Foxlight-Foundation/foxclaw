...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): FoxClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as FoxClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): FoxClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FoxClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: FoxClawConfig, accountId = "media") {
    await handleTelegramAction(
