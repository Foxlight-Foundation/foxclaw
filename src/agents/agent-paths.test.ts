...
describe("resolveFoxClawAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveFoxClawAgentDir();
  ...
  it("honors FOXCLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          FOXCLAW_STATE_DIR: undefined,
          FOXCLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveFoxClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when FOXCLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers FOXCLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          FOXCLAW_STATE_DIR: undefined,
          FOXCLAW_AGENT_DIR: primaryOverride,
