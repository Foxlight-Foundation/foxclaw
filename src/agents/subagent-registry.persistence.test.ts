...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["FOXCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-subagent-"));
  ...
  it("uses isolated temp state when FOXCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "foxclaw-test-state"));
  });
});
