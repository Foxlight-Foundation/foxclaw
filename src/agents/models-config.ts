...
import { planFoxClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: FoxClawConfig): {
  config: FoxClawConfig;
  sourceConfigForSecrets: FoxClawConfig;
} {
...
export async function ensureFoxClawModelsJson(
  config?: FoxClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveFoxClawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planFoxClawModelsJson({
      cfg,
