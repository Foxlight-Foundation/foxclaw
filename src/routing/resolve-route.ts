...
function listAgents(cfg: FoxClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: FoxClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<FoxClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: FoxClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: FoxClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FoxClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<FoxClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  FoxClawConfig,
  {
    bindingsRef: FoxClawConfig["bindings"];
    agentsRef: FoxClawConfig["agents"];
    sessionRef: FoxClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: FoxClawConfig): Map<string, ResolvedAgentRoute> {
