...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: FoxClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: FoxClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/foxclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<FoxClawConfig["bindings"]>[number];
