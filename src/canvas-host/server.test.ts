...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("foxclawCanvasA2UIAction");
    expect(out).toContain("foxclawSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("foxclawSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.foxclawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__foxclaw__/a2ui/a2ui.bundle.js`,
      );
