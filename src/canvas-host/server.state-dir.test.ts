...
describe("canvas host state dir defaults", () => {
  it("uses FOXCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("foxclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("FoxClaw Canvas");
      } finally {
        await handler.close();
      }
    });
