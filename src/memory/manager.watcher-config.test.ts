...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): FoxClawConfig {
    const defaults: NonNullable<NonNullable<FoxClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: FoxClawConfig) {
