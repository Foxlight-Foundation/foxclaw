...
function telegramCfg(): FoxClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as FoxClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: FoxClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as FoxClawConfig);
...
function createSignalAccountOverrideCfg(): FoxClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as FoxClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as FoxClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as FoxClawConfig,
        accountId: undefined,
        params: {
