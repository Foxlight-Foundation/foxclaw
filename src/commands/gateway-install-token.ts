...
import { readConfigFileSnapshot, writeConfigFile, type FoxClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("foxclaw config set gateway.auth.mode token")} or ${formatCliCommand("foxclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
