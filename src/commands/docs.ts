...
const SEARCH_TOOL = "https://docs.foxclaw.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.foxclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('foxclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.foxclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('foxclaw docs "your query"')}`);
    }
    return;
  }
