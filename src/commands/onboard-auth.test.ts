...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FOXCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("foxclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-oauth-sync-"));
    ...
    process.env.FOXCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-oauth-nosync-"));
    process.env.FOXCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FOXCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-oauth-external-"));
    process.env.FOXCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FOXCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FOXCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("foxclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
