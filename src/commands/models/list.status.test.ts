...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFoxClawAgentDir: vi.fn().mockReturnValue("/tmp/foxclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/foxclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/foxclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/foxclaw-dev/foxclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFoxClawAgentDir: mocks.resolveFoxClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/foxclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFoxClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/foxclaw-dev/foxclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/foxclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/foxclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/foxclaw-agent-custom");
