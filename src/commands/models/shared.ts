...
export async function loadValidConfigOrThrow(): Promise<FoxClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: FoxClawConfig) => FoxClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FoxClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FoxClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("foxclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
