...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFoxClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("foxclaw models auth setup-token")}\` or \`${formatCliCommand("foxclaw configure")}\`.`
          : `Run \`${formatCliCommand("foxclaw configure")}\` or set an API key env var.`;
