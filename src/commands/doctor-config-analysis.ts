...
import { FoxClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: FoxClawConfig): {
  config: FoxClawConfig;
  removed: string[];
} {
  const parsed = FoxClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: FoxClawConfig): void {
