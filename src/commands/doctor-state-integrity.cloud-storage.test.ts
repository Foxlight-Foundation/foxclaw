...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "FoxClaw", ".foxclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/foxclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".foxclaw");
  ...
  it("anchors cloud detection to OS homedir when FOXCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".foxclaw");
    const originalFoxClawHome = process.env.FOXCLAW_HOME;
    process.env.FOXCLAW_HOME = "/tmp/foxclaw-home-override";
