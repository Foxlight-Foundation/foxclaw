...
function rawKeyValue(config: FoxClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: FoxClawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: FoxClawConfig, provider: SearchProvider): FoxClawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: FoxClawConfig, result: FoxClawConfig): FoxClawConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with foxclaw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — FoxClaw will store a reference instead of the API key.",
