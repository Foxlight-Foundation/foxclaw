...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByFoxClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByFoxClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("FoxClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`foxclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("foxclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("foxclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("foxclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("foxclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.foxclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.foxclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("foxclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("foxclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("foxclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("foxclaw gateway probe")}`);
  }
}
