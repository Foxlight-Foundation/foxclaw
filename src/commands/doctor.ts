...
import { resolveFoxClawPackageRoot } from "../infra/foxclaw-root.js";
...
function resolveMode(cfg: FoxClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("FoxClaw doctor");

  const root = await resolveFoxClawPackageRoot({
  ...
  let cfg: FoxClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("foxclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("foxclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("foxclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("foxclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("foxclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("foxclaw doctor --fix")}" to apply changes.`);
  }
