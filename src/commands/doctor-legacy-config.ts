...
export function normalizeCompatibilityConfigValues(cfg: FoxClawConfig): {
  config: FoxClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: FoxClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as FoxClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as FoxClawConfig["browser"],
    };
    changes.push(
