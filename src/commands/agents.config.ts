...
function resolveAgentName(cfg: FoxClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: FoxClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FoxClawConfig): AgentSummary[] {
