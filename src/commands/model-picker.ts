...
type PromptDefaultModelResult = { model?: string; config?: FoxClawConfig };
...
function resolveConfiguredModelRaw(cfg: FoxClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: FoxClawConfig): string[] {
...
export function applyPrimaryModel(cfg: FoxClawConfig, model: string): FoxClawConfig {
...
export function applyModelAllowlist(cfg: FoxClawConfig, models: string[]): FoxClawConfig {
  const defaults = cfg.agents?.defaults;
