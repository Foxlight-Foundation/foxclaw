...
type EnvSnapshot = {
  HOME?: string;
  FOXCLAW_HOME?: string;
  FOXCLAW_STATE_DIR?: string;
  FOXCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    FOXCLAW_HOME: process.env.FOXCLAW_HOME,
    FOXCLAW_STATE_DIR: process.env.FOXCLAW_STATE_DIR,
    FOXCLAW_OAUTH_DIR: process.env.FOXCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: FoxClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: FoxClawConfig) {
...
async function runStateIntegrityText(cfg: FoxClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "foxclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.FOXCLAW_HOME = tempHome;
    process.env.FOXCLAW_STATE_DIR = path.join(tempHome, ".foxclaw");
    delete process.env.FOXCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.FOXCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when FOXCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.FOXCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: FoxClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints foxclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: FoxClawConfig = {};
    ...
    expect(text).toMatch(/foxclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/foxclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /foxclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
