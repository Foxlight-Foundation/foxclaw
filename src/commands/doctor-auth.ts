...
function pruneAuthProfiles(
  cfg: FoxClawConfig,
  profileIds: Set<string>,
): { next: FoxClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("foxclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("foxclaw models auth setup-token")} or ${formatCliCommand(
      "foxclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "foxclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("foxclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("foxclaw configure")}\` or \`${formatCliCommand("foxclaw onboard")}\`.`;
}
