...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: FoxClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: FoxClawConfig) => { changed: boolean; next: FoxClawConfig };
  defaultModel: string;
  overrideConfig: FoxClawConfig;
  alreadyDefaultConfig: FoxClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: FoxClawConfig) => config,
      applyDefaultConfig: (config: FoxClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: FoxClawConfig = { agents: { defaults: {} } };
