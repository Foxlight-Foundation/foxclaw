...
import type { FoxClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: FoxClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: FoxClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("foxclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.foxclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("foxclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("foxclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("foxclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("foxclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("foxclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('foxclaw config set session.dmScope "per-channel-peer"') +
