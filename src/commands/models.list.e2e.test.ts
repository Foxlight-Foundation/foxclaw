...
const ensureFoxClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveFoxClawAgentDir = vi.fn().mockReturnValue("/tmp/foxclaw-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureFoxClawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveFoxClawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureFoxClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureFoxClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureFoxClawModelsJson).toHaveBeenCalled();
    expect(ensureFoxClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
