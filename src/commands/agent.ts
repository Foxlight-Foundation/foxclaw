...
function prependInternalEventContext(
  ...
  if (body.includes("FoxClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("foxclaw agents list")}" to see configured agents.`,
      );
    }
  }
