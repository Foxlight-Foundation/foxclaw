...
beforeAll(() => {
  envSnapshot = captureEnv(["FOXCLAW_PROFILE"]);
  process.env.FOXCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/foxclaw-root.js", () => ({
  resolveFoxClawPackageRoot: vi.fn().mockResolvedValue("/tmp/foxclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/foxclaw/pnpm-lock.yaml",
      markerPath: "/tmp/foxclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.foxclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.foxclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("foxclaw status --all") ||
          line.includes("foxclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("FOXCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
