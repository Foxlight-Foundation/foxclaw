...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "FOXCLAW_GATEWAY_TOKEN",
          placeholder: "FOXCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like FOXCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. FoxClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: FoxClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.foxclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
