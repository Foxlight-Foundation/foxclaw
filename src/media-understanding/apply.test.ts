...
const TEMP_MEDIA_PREFIX = "foxclaw-media-";
...
function createGroqAudioConfig(): FoxClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): FoxClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): FoxClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: FoxClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredFoxClawTmpDir();
