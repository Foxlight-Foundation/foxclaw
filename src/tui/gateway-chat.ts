...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set FOXCLAW_GATEWAY_TOKEN/FOXCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.FOXCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.FOXCLAW_GATEWAY_PASSWORD);
