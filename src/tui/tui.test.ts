...
describe("resolveInitialTuiAgentId", () => {
  const cfg: FoxClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/foxclaw" },
        { id: "ops", workspace: "/tmp/foxclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/foxclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run foxclaw devices list");
    expect(state.pairingHint).toContain("foxclaw devices list");
  });
