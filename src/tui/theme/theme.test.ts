...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      FOXCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when FOXCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ FOXCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when FOXCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ FOXCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats FOXCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ FOXCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("FOXCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      FOXCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ FOXCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ FOXCLAW_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.FOXCLAW_THEME = "light";
