...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing foxclaw.extensions; update the plugin package to include foxclaw.extensions (for example ["./dist/index.js"]). See https://docs.foxclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-foxclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_FOXCLAW_EXTENSIONS: "missing_foxclaw_extensions",
  EMPTY_FOXCLAW_EXTENSIONS: "empty_foxclaw_extensions",
...
function ensureFoxClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_FOXCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json foxclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_FOXCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureFoxClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from foxclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-foxclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "foxclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "foxclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "foxclaw-npm-pack-",
    spec,
    timeoutMs,
