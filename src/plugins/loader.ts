...
import { resolveFoxClawPackageRootSync } from "../infra/foxclaw-root.js";
...
import { discoverFoxClawPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`foxclaw/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FoxClawPluginDefinition;
  register?: FoxClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FoxClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FoxClawPluginDefinition;
...
export function loadFoxClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverFoxClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "foxclaw/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FoxClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as FoxClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
