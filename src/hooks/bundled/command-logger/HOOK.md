...
homepage: https://docs.foxclaw.ai/automation/hooks#command-logger
metadata:
  {
    "foxclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.foxclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.foxclaw/logs/commands.log ~/.foxclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/foxclaw`:
   ```
   /home/username/.foxclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
