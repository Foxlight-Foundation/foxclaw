...
This directory contains hooks that ship with FoxClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.foxclaw/foxclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'foxclaw.*gateway' && pnpm foxclaw gateway`
3. Enable the hook: `foxclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.foxclaw.ai/automation/hooks
