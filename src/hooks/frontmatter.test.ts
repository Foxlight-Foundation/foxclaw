...
`;
    ...
    expect(parsed.foxclaw.emoji).toBe("💾");
    expect(parsed.foxclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.foxclaw.emoji).toBe("📝");
    expect(parsed.foxclaw.events).toEqual(["command"]);
    expect(parsed.foxclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.foxclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"foxclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"foxclaw": {"events": ["test"]}}');
  });
...
describe("resolveFoxClawMetadata", () => {
  it("extracts foxclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveFoxClawMetadata(frontmatter);
  ...
  it("returns undefined when foxclaw key is missing", () => {
    const frontmatter = {
...
`;
    ...
    const foxclaw = resolveFoxClawMetadata(frontmatter);
    expect(foxclaw).toBeDefined();
    expect(foxclaw?.emoji).toBe("💾");
    expect(foxclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(foxclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(foxclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const foxclaw = resolveFoxClawMetadata(frontmatter);
    expect(foxclaw?.emoji).toBe("disk");
    expect(foxclaw?.events).toEqual(["command:new"]);
  });
});
