...
const resolveWebMediaOptions = (params: {
...
export const getDefaultLocalRoots = (): readonly string[] => {
  return getDefaultMediaLocalRoots();
};

const assertLocalMediaAllowed = async (
  mediaPath: string,
...
const formatMb = (bytes: number, digits = 2): string => {
  return (bytes / MB).toFixed(digits);
};

const formatCapLimit = (label: string, cap: number, size: number): string => {
  return `${label} exceeds ${formatMb(cap, 0)}MB limit (got ${formatMb(size)}MB)`;
};

const formatCapReduce = (label: string, cap: number, size: number): string => {
  return `${label} could not be reduced below ${formatMb(cap, 0)}MB (got ${formatMb(size)}MB)`;
};

const isHeicSource = (opts: { contentType?: string; fileName?: string }): boolean => {
...
const toJpegFileName = (fileName?: string): string | undefined => {
  if (!fileName) {
    return undefined;
  }
...
const logOptimizedImage = (params: { originalSize: number; optimized: OptimizedImage }): void => {
  if (!shouldLogVerbose()) {
    return;
  }
...
const optimizeImageWithFallback = async (params: {
  buffer: Buffer;
  cap: number;
...
const loadWebMediaInternal = async (
  mediaUrl: string,
...
export const loadWebMedia = async (
  mediaUrl: string,
  maxBytesOrOptions?: number | WebMediaOptions,
  options?: { ssrfPolicy?: SsrFPolicy; localRoots?: readonly string[] | "any" },
): Promise<WebMediaResult> => {
...
export const loadWebMediaRaw = async (
  mediaUrl: string,
  maxBytesOrOptions?: number | WebMediaOptions,
  options?: { ssrfPolicy?: SsrFPolicy; localRoots?: readonly string[] | "any" },
): Promise<WebMediaResult> => {
...
export const optimizeImageToJpeg = async (
  buffer: Buffer,
  maxBytes: number,
