...
import { scheduleGatewaySigusr1Restart, triggerFoxClawRestart } from "../../infra/restart.js";
...
export const handleSessionCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: "⚠️ /session idle and /session max-age are not available on this channel.",
    },
  };
};
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting FoxClaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFoxClawRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting FoxClaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
