...
const SLACK_REPLY_BUTTON_ACTION_ID = "foxclaw:reply_button";
const SLACK_REPLY_SELECT_ACTION_ID = "foxclaw:reply_select";
...
function buildButtonsBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `foxclaw_reply_buttons_${index}`,
...
function buildSelectBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `foxclaw_reply_select_${index}`,
    elements: [
      {
        type: "static_select",
