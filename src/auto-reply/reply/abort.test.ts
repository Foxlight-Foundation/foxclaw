...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop foxclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@foxclaw_bot", { botUsername: "foxclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@foxclaw_bot", { botUsername: "foxclaw_bot" })).toBe(true);
