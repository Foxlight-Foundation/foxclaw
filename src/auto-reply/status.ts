...
type AgentDefaults = NonNullable<NonNullable<FoxClawConfig["agents"]>["defaults"]>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦊 FoxClaw ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: FoxClawConfig): string {
  const lines = ["ℹ️ Help", ""];
