...
export function isCommandEnabled(cfg: FoxClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FoxClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as FoxClawConfig),
...
export function getCommandDetection(_cfg?: FoxClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FoxClawConfig) {
