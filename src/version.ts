...
declare const __FOXCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "foxclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["FOXCLAW_VERSION"],
      runtimeVersion,
      env["FOXCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current FoxClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __FOXCLAW_VERSION__ === "string" ? __FOXCLAW_VERSION__ : undefined,
  bundledVersion: process.env.FOXCLAW_BUNDLED_VERSION,
});
