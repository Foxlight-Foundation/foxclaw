...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "foxclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".foxclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<FoxClawConfig> = {},
): FoxClawConfig {
  const base: FoxClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<FoxClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<FoxClawConfig>,
