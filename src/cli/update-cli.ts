...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update FoxClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["foxclaw update", "Update a source checkout (git)"],
        ["foxclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["foxclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["foxclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["foxclaw update --dry-run", "Preview actions without changing anything"],
        ["foxclaw update --no-restart", "Update without restarting the service"],
        ["foxclaw update --json", "Output result as JSON"],
        ["foxclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["foxclaw update wizard", "Interactive update wizard"],
        ["foxclaw --update", "Shorthand for foxclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run foxclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.foxclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["foxclaw update status", "Show channel + version status."],
          ["foxclaw update status --json", "JSON output."],
          ["foxclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.foxclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
