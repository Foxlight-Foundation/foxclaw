...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    FOXCLAW_STATE_DIR: "/tmp/foxclaw-daemon",
    FOXCLAW_CONFIG_PATH: "/tmp/foxclaw-daemon/foxclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.FOXCLAW_STATE_DIR ?? "/tmp/foxclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.FOXCLAW_CONFIG_PATH ?? `${stateDir}/foxclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/foxclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FOXCLAW_STATE_DIR = "/tmp/foxclaw-cli";
    process.env.FOXCLAW_CONFIG_PATH = "/tmp/foxclaw-cli/foxclaw.json";
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "foxclaw-gateway" }],
        hints: [],
      },
      healthy: false,
