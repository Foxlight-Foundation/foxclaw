    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["foxclaw nodes status", "List known nodes with live status."],
          ["foxclaw nodes pairing pending", "Show pending node pairing requests."],
          ['foxclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["foxclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.foxclaw.ai/cli/nodes")}\n`,
    );
