...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.foxclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.FOXCLAW_PROFILE = profile;

  const stateDir = env.FOXCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.FOXCLAW_STATE_DIR?.trim()) {
    env.FOXCLAW_STATE_DIR = stateDir;
  }

  if (!env.FOXCLAW_CONFIG_PATH?.trim()) {
    env.FOXCLAW_CONFIG_PATH = path.join(stateDir, "foxclaw.json");
  }

  if (profile === "dev" && !env.FOXCLAW_GATEWAY_PORT?.trim()) {
    env.FOXCLAW_GATEWAY_PORT = "19001";
  }
}
