...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "foxclaw";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "foxclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "foxclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option(
      "--driver <driver>",
      "Profile driver (foxclaw|extension|existing-session). Default: foxclaw",
    )
    .action(
