...
const EXAMPLES = [
  ...
  ["foxclaw gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["foxclaw --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["foxclaw gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["foxclaw gateway ...", "Gateway control via WebSocket."],
  [
    'foxclaw agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'foxclaw message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.foxclaw-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates FOXCLAW_STATE_DIR/FOXCLAW_CONFIG_PATH under ~/.foxclaw-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (
    ...
    console.log(
      commit ? `FoxClaw ${ctx.programVersion} (${commit})` : `FoxClaw ${ctx.programVersion}`,
    );
    process.exit(0);
  }
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.foxclaw.ai/cli");
