...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@foxclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@foxclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@foxclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@foxclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @foxclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@foxclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@foxclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/foxclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@foxclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @foxclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@foxclaw/plugin-alpha@latest",
      true,
      "/tmp/foxclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@foxclaw/plugin-alpha@latest",
      installPath: "/tmp/foxclaw/extensions/alpha",
      version: "1.2.3",
