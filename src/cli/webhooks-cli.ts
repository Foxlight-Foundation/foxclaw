...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.foxclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FoxClaw hooks")
    ...
    .option("--hook-url <url>", "FoxClaw hook URL")
    .option("--hook-token <token>", "FoxClaw hook token")
