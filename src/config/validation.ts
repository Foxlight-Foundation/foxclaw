...
import type { FoxClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FoxClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: FoxClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: FoxClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FoxClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FoxClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FoxClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as FoxClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as FoxClawConfig,
  };
}
