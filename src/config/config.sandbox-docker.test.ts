...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("foxclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "foxclaw-sandbox-browser-global" },
      agentBrowser: { network: "foxclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("foxclaw-sandbox-browser-agent");
  });
