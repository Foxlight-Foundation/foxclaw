...
describe("config identity defaults", () => {
  ...
  it("does not derive mention defaults and only sets ackReactionScope when identity is present", async () => {
    await withTempHome("foxclaw-config-identity-", async (home) => {
  ...
  it("does not override explicit values", async () => {
    await withTempHome("foxclaw-config-identity-", async (home) => {
      ...
      expect(cfg.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@foxclaw"]);
    });
  });
  ...
  it("supports provider textChunkLimit config", async () => {
    await withTempHome("foxclaw-config-identity-", async (home) => {
      ...
      expect(cfg.channels?.slack?.textChunkLimit).toBe(3333);
