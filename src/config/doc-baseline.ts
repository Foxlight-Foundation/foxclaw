...
function resolveRepoRoot(): string {
  const fromPackage = resolveFoxClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    FOXCLAW_STATE_DIR: path.join(os.tmpdir(), "foxclaw-config-doc-baseline-state"),
    FOXCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
