...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): FoxClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: FoxClawConfig, env: NodeJS.ProcessEnv) =>
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".foxclaw/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".foxclaw/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });
