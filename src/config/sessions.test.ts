...
describe("sessions", () => {
  ...
  const sessionsTestRegistry = createTestRegistry([
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-sessions-suite-"));
  });

  beforeEach(() => {
    setActivePluginRegistry(sessionsTestRegistry);
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ FOXCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-foxclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-foxclaw#general");
  });
  ...
  it("derives session transcripts dir from FOXCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FOXCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when FOXCLAW_STATE_DIR differs from stored paths", () => {
