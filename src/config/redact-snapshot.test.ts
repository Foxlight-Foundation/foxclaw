...
import type { ConfigFileSnapshot } from "./types.foxclaw.js";
import { FoxClawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(FoxClawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.foxclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/foxclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/foxclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/foxclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/foxclaw/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = FoxClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "FoxClawConfig";
    const hints = mainSchemaHints;
