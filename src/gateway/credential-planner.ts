...
type GatewaySecretDefaults = NonNullable<FoxClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${FOXCLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(env, ["FOXCLAW_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN"], includeLegacyEnv);
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["FOXCLAW_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
