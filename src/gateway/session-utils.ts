...
import { type FoxClawConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: FoxClawConfig): string[] {
...
export function listAgentsForGateway(cfg: FoxClawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: FoxClawConfig): string {
...
function resolveSessionStoreAgentId(cfg: FoxClawConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: FoxClawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: FoxClawConfig): GatewaySessionsDefaults {
