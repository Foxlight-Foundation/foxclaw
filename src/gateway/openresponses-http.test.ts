...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("FOXCLAW_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "foxclaw", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "foxclaw", input: "hi" },
        { "x-foxclaw-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "foxclaw:beta", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "foxclaw", input: "hi" },
        { "x-foxclaw-message-channel": "custom-client-channel" },
      );
