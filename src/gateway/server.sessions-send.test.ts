...
const { createFoxClawTools } = await import("../agents/foxclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createFoxClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createFoxClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["FOXCLAW_GATEWAY_PORT", "FOXCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.FOXCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.FOXCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("FOXCLAW_CONFIG_PATH missing in gateway test environment");
      }
