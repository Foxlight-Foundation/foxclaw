...
export async function withTempConfig(params: {
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.FOXCLAW_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.FOXCLAW_DISABLE_CONFIG_CACHE;
    } else {
      process.env.FOXCLAW_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
