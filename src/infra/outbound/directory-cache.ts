...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FoxClawConfig | null = null;
  ...
  get(key: string, cfg: FoxClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FoxClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: FoxClawConfig): void {
