...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FoxClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FoxClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: FoxClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: FoxClawConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: FoxClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: FoxClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FoxClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FoxClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
