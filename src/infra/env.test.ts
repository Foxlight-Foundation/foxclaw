...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    withEnv(
      {
        ...
        FOXCLAW_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "FOXCLAW_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: FOXCLAW_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        FOXCLAW_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "FOXCLAW_BLANK_ENV",
