...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __foxclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__foxclawDiagnosticEventsState) {
    globalStore.__foxclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__foxclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: FoxClawConfig): boolean {
