const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "FOXCLAW_LAUNCHD_LABEL"],
  systemd: ["FOXCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["FOXCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "FOXCLAW_SERVICE_MARKER",
  "FOXCLAW_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.FOXCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.FOXCLAW_SERVICE_KIND?.trim();
