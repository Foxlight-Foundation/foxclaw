...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FOXCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*foxclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "foxclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "foxclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "foxclaw-gw");
  ...
  records.push(`_foxclaw-gw._tcp IN PTR ${instanceLabel}._foxclaw-gw._tcp`);
  records.push(`${instanceLabel}._foxclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._foxclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; foxclaw-content-hash: ${contentHash}\n${contentBody}`;
}
