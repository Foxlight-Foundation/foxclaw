...
function candidateBinDirs(opts: EnsureFoxClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `foxclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "foxclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FOXCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "foxclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `foxclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFoxClawCliOnPath(opts: EnsureFoxClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FOXCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
