...
export function triggerFoxClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.FOXCLAW_SYSTEMD_UNIT,
      process.env.FOXCLAW_PROFILE,
    );
  ...
  const label =
    process.env.FOXCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FOXCLAW_PROFILE);
