...
describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when FOXCLAW_NO_RESPAWN is set", () => {
    process.env.FOXCLAW_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
  ...
  it("spawns detached child with current exec argv", () => {
    delete process.env.FOXCLAW_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when FOXCLAW_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });
