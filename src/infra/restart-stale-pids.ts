 ...
 * returning control to the caller (typically `triggerFoxClawRestart` →
...
/**
 * Parse foxclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("foxclaw")) {
 ...
 * Returns only PIDs that belong to foxclaw gateway processes (not the current process).
 */
