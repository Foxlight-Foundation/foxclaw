...
describe("npm registry spec validation", () => {
  it("accepts bare package names, exact versions, and dist-tags", () => {
    expect(validateRegistryNpmSpec("@foxclaw/voice-call")).toBeNull();
    expect(validateRegistryNpmSpec("@foxclaw/voice-call@1.2.3")).toBeNull();
    expect(validateRegistryNpmSpec("@foxclaw/voice-call@1.2.3-beta.4")).toBeNull();
    expect(validateRegistryNpmSpec("@foxclaw/voice-call@latest")).toBeNull();
    expect(validateRegistryNpmSpec("@foxclaw/voice-call@beta")).toBeNull();
  });

  it("rejects semver ranges", () => {
    expect(validateRegistryNpmSpec("@foxclaw/voice-call@^1.2.3")).toContain(
  ...
  it("rejects unsupported registry protocols and malformed selectors", () => {
    ...
    expect(validateRegistryNpmSpec("git+ssh://github.com/foxclaw/foxclaw")).toContain(
...
describe("npm registry spec parsing helpers", () => {
  it("parses bare, tag, and exact prerelease specs", () => {
    expect(parseRegistryNpmSpec("@foxclaw/voice-call")).toEqual({
      name: "@foxclaw/voice-call",
      raw: "@foxclaw/voice-call",
      selectorKind: "none",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@foxclaw/voice-call@beta")).toEqual({
      name: "@foxclaw/voice-call",
      raw: "@foxclaw/voice-call@beta",
      selector: "beta",
      selectorKind: "tag",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@foxclaw/voice-call@1.2.3-beta.1")).toEqual({
      name: "@foxclaw/voice-call",
      raw: "@foxclaw/voice-call@1.2.3-beta.1",
...
describe("npm prerelease resolution policy", () => {
  it("blocks prerelease resolutions for bare specs", () => {
    const spec = parseRegistryNpmSpec("@foxclaw/voice-call");
  ...
  it("blocks prerelease resolutions for latest", () => {
    const spec = parseRegistryNpmSpec("@foxclaw/voice-call@latest");
    expect(spec).not.toBeNull();
    expect(
      isPrereleaseResolutionAllowed({
        spec: spec!,
  ...
  it("allows prerelease resolutions when the user explicitly opted in", () => {
    const tagSpec = parseRegistryNpmSpec("@foxclaw/voice-call@beta");
    const versionSpec = parseRegistryNpmSpec("@foxclaw/voice-call@1.2.3-beta.1");
  ...
  it("allows stable resolutions even for bare and latest specs", () => {
    const bareSpec = parseRegistryNpmSpec("@foxclaw/voice-call");
    const latestSpec = parseRegistryNpmSpec("@foxclaw/voice-call@latest");
  ...
  it("formats prerelease resolution guidance based on selector intent", () => {
    ...
    ).toContain(`Use "@foxclaw/voice-call@beta"`);
    expect(
      formatPrereleaseResolutionError({
        spec: tagSpec!,
        resolvedVersion: "1.2.3-rc.1",
