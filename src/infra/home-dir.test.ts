...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for FOXCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { FOXCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when FOXCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ FOXCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
