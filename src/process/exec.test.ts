...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        FOXCLAW_BASE_ENV: "base",
        FOXCLAW_TO_REMOVE: undefined,
      },
      env: {
        FOXCLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.FOXCLAW_BASE_ENV).toBe("base");
    expect(resolved.FOXCLAW_TEST_ENV).toBe("ok");
    expect(resolved.FOXCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.FOXCLAW_CLI).toBe(FOXCLAW_CLI_ENV_VALUE);
  });
