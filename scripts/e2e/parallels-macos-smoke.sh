...
INSTALL_URL="https://foxclaw.ai/install.sh"
HOST_PORT="18425"
...
CHECK_LATEST_REF=1
JSON_OUTPUT=0
GUEST_FOXCLAW_BIN="/opt/homebrew/bin/foxclaw"
GUEST_FOXCLAW_ENTRY="/opt/homebrew/lib/node_modules/foxclaw/foxclaw.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/foxclaw-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://foxclaw.ai/install.sh
...
spawn {*}$cmd
send -- "printf '__FOXCLAW_READY__\\n'\r"
expect "__FOXCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/foxclaw-prl.sh <<'__FOXCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__FOXCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/foxclaw-prl.sh; rc=\$?; rm -f /tmp/foxclaw-prl.sh; printf '__FOXCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__FOXCLAW_RC__:(-?[0-9]+)} {
...
export FOXCLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/foxclaw-install.sh
bash /tmp/foxclaw-install.sh${version_arg_q}
$GUEST_FOXCLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_FOXCLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/foxclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/foxclaw"; check_path "\$root/foxclaw/extensions"; if [ -d "\$root/foxclaw/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/foxclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_FOXCLAW_ENTRY" agent --agent main --message ping --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "foxclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "foxclaw-main-upgrade.tgz"
