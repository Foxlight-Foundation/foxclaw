...
IMAGE_NAME="foxclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="foxclaw-net-e2e-$$"
GW_NAME="foxclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "FOXCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "FOXCLAW_SKIP_CHANNELS=1" \
  -e "FOXCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "FOXCLAW_SKIP_CRON=1" \
  -e "FOXCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
