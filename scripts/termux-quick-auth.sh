...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/foxclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FoxClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/foxclaw/scripts/mobile-reauth.sh'" --id foxclaw-auth
        ;;
    *)
