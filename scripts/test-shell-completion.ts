 ...
 * `foxclaw update`. Use it to verify the completion installation flow
...
\`foxclaw update\`, \`foxclaw doctor\`, and \`foxclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FoxClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`foxclaw completion --install\` later to enable.`));
    return;
  }
