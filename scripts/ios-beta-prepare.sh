...
- regenerates apps/ios/FoxClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${FOXCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid FOXCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid FOXCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid FOXCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid FOXCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing FOXCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(FOXCLAW_URL_SLASH)$(FOXCLAW_URL_SLASH)#g'
)"
...
FOXCLAW_CODE_SIGN_STYLE = Automatic
FOXCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
FOXCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
FOXCLAW_APP_BUNDLE_ID = ai.foxclaw.client
FOXCLAW_SHARE_BUNDLE_ID = ai.foxclaw.client.share
FOXCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.foxclaw.client.activitywidget
FOXCLAW_WATCH_APP_BUNDLE_ID = ai.foxclaw.client.watchkitapp
FOXCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.foxclaw.client.watchkitapp.extension
FOXCLAW_APP_PROFILE =
FOXCLAW_SHARE_PROFILE =
FOXCLAW_PUSH_TRANSPORT = relay
FOXCLAW_PUSH_DISTRIBUTION = official
FOXCLAW_URL_SLASH = /
FOXCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
FOXCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
