#!/usr/bin/env bash
# ClawDock - Docker helpers for FoxClaw
# Inspired by Simon Willison's "Running FoxClaw in Docker"
# https://til.simonwillison.net/llms/foxclaw-docker
#
# Installation:
...
# Common paths to check for FoxClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/foxclaw"
  "${HOME}/workspace/foxclaw"
  "${HOME}/projects/foxclaw"
  "${HOME}/dev/foxclaw"
  "${HOME}/code/foxclaw"
  "${HOME}/src/foxclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found FoxClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/foxclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ FoxClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/foxclaw/foxclaw.git ~/foxclaw"
    echo "  cd ~/foxclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FOXCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d foxclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart foxclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f foxclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.foxclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec foxclaw-gateway \
    bash -c 'echo "alias foxclaw=\"./foxclaw.mjs\"" > /tmp/.bashrc_foxclaw && bash --rcfile /tmp/.bashrc_foxclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm foxclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build foxclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FOXCLAW_GATEWAY_TOKEN=$token" foxclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" foxclaw-gateway \
    bash -c './foxclaw.mjs config set gateway.remote.token "$TOKEN" && ./foxclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec foxclaw-gateway \
    bash -c "./foxclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart foxclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm foxclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec foxclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'foxclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for FoxClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (foxclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to foxclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.foxclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'foxclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'foxclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.foxclaw.ai${_CLR_RESET}"
  echo ""
}
