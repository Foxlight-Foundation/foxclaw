...
function stripWorkspaceFoxclawDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.foxclaw !== "workspace:*") {
    return false;
  }
  delete devDeps.foxclaw;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceFoxclawDevDependency(pkg);
