...
BASE_IMAGE="${BASE_IMAGE:-foxclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-foxclaw-sandbox-common:bookworm-slim}"
...
FOXCLAW_DOCKER_BUILD_USE_BUILDX="${FOXCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
FOXCLAW_DOCKER_BUILD_CACHE_FROM="${FOXCLAW_DOCKER_BUILD_CACHE_FROM:-}"
FOXCLAW_DOCKER_BUILD_CACHE_TO="${FOXCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${FOXCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${FOXCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${FOXCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${FOXCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${FOXCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=foxclaw.sandbox=1)
NOTE
