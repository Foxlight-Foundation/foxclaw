...
# FoxClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one FoxClaw."
...
GUM_VERSION="${FOXCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 FoxClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://foxclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$FOXCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.foxclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_foxclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.foxclaw-* "$npm_root"/foxclaw 2>/dev/null || true
}

extract_foxclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_foxclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/foxclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/foxclaw"|"/usr/local/bin/foxclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/foxclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale foxclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing foxclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing FoxClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_foxclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "FoxClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*foxclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_foxclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_foxclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "FoxClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an foxclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "FOXCLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "FOXCLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "FOXCLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "FOXCLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "FOXCLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "FOXCLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "FOXCLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "FOXCLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "FOXCLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "FOXCLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "FOXCLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "FOXCLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "FOXCLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${FOXCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${FOXCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((FOXCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${FOXCLAW_NO_ONBOARD:-0}
NO_PROMPT=${FOXCLAW_NO_PROMPT:-0}
DRY_RUN=${FOXCLAW_DRY_RUN:-0}
INSTALL_METHOD=${FOXCLAW_INSTALL_METHOD:-}
FOXCLAW_VERSION=${FOXCLAW_VERSION:-latest}
USE_BETA=${FOXCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/foxclaw"
GIT_DIR=${FOXCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${FOXCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${FOXCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${FOXCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${FOXCLAW_VERIFY_INSTALL:-0}"
FOXCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
FoxClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/foxclaw)
...
Environment variables:
  FOXCLAW_INSTALL_METHOD=git|npm
  FOXCLAW_VERSION=latest|next|<semver>
  FOXCLAW_BETA=0|1
  FOXCLAW_GIT_DIR=...
  FOXCLAW_GIT_UPDATE=0|1
  FOXCLAW_NO_PROMPT=1
  FOXCLAW_VERIFY_INSTALL=1
  FOXCLAW_DRY_RUN=1
  FOXCLAW_NO_ONBOARD=1
  FOXCLAW_VERBOSE=1
  FOXCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected FoxClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a FoxClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_foxclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"foxclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://foxclaw.ai/install.sh | bash"
}
...
ensure_foxclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/foxclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/foxclaw" ]]; then
        ln -sf "$npm_root/foxclaw/dist/entry.js" "${npm_bin}/foxclaw"
        ui_info "Created foxclaw bin link at ${npm_bin}/foxclaw"
    fi
    return 0
}

# Check for existing FoxClaw installation
check_existing_foxclaw() {
    if [[ -n "$(type -P foxclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing FoxClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make foxclaw show as \"command not found\" in new terminals."
...
warn_foxclaw_not_found() {
    ui_warn "Installed, but foxclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t foxclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named foxclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_foxclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P foxclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/foxclaw" ]]; then
        echo "${npm_bin}/foxclaw"
        return 0
    fi
...
install_foxclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/foxclaw/foxclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing FoxClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing FoxClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning FoxClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building FoxClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/foxclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/foxclaw"
    ui_success "FoxClaw wrapper installed to \$HOME/.local/bin/foxclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view foxclaw dist-tags.beta 2>/dev/null || true)"
...
install_foxclaw() {
    local package_name="foxclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            FOXCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="foxclaw"
        else
            FOXCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${FOXCLAW_VERSION}" ]]; then
        FOXCLAW_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${FOXCLAW_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing FoxClaw v${resolved_version}"
    else
        ui_info "Installing FoxClaw (${FOXCLAW_VERSION})"
    fi
    local install_spec=""
    if [[ "${FOXCLAW_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${FOXCLAW_VERSION}"
    fi

    if ! install_foxclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_foxclaw_paths
        install_foxclaw_npm "${install_spec}"
    fi

    if [[ "${FOXCLAW_VERSION}" == "latest" && "${package_name}" == "foxclaw" ]]; then
        if ! resolve_foxclaw_bin &> /dev/null; then
            ui_warn "npm install foxclaw@latest failed; retrying foxclaw@next"
            cleanup_npm_foxclaw_paths
            install_foxclaw_npm "foxclaw@next"
        fi
    fi

    ensure_foxclaw_bin_link || true

    ui_success "FoxClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${FOXCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_foxclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (foxclaw not on PATH yet)"
        warn_foxclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${FOXCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.foxclaw/workspace-${profile}"
    else
        echo "${HOME}/.foxclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${FOXCLAW_CONFIG_PATH:-$HOME/.foxclaw/foxclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run foxclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but foxclaw not on PATH; skipping onboarding"
        warn_foxclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run foxclaw onboard to retry"
        return
    }
}
...
resolve_foxclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${FOXCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v foxclaw &> /dev/null; then
        claw="$(command -v foxclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_foxclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/foxclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/foxclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: foxclaw not on PATH yet"
        warn_foxclaw_not_found
        return 1
    fi

    run_quiet_step "Checking FoxClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: foxclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_foxclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found FoxClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set FOXCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_foxclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing FoxClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g foxclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g foxclaw 2>/dev/null || true
        ...
        install_foxclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/foxclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/foxclaw"
    ...
    FOXCLAW_BIN="$(resolve_foxclaw_bin || true)"
    ...
    installed_version=$(resolve_foxclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 FoxClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 FoxClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/foxclaw"
        ui_kv "Update command" "foxclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://foxclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running foxclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                FOXCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                FOXCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                FOXCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run foxclaw doctor and foxclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run foxclaw onboard later"
        else
            local config_path="${FOXCLAW_CONFIG_PATH:-$HOME/.foxclaw/foxclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (foxclaw not on PATH yet)"
                    warn_foxclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run foxclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v foxclaw &> /dev/null; then
        local claw="${FOXCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_foxclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (foxclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if FOXCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: foxclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${FOXCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
