...
#
# Suggested cron: */30 * * * * /home/admin/foxclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FoxClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., foxclaw-alerts)
...
STATE_FILE="$HOME/.foxclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FoxClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use foxclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FoxClaw to $NOTIFY_PHONE..."
            foxclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FoxClaw is down. Run: ssh l36 '~/foxclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
