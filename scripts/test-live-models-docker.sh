...
docker run --rm -t \
  ...
  -e FOXCLAW_LIVE_MODELS="${FOXCLAW_LIVE_MODELS:-${CLAWDBOT_LIVE_MODELS:-modern}}" \
  -e FOXCLAW_LIVE_PROVIDERS="${FOXCLAW_LIVE_PROVIDERS:-${CLAWDBOT_LIVE_PROVIDERS:-}}" \
  -e FOXCLAW_LIVE_MAX_MODELS="${FOXCLAW_LIVE_MAX_MODELS:-${CLAWDBOT_LIVE_MAX_MODELS:-48}}" \
  -e FOXCLAW_LIVE_MODEL_TIMEOUT_MS="${FOXCLAW_LIVE_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e FOXCLAW_LIVE_REQUIRE_PROFILE_KEYS="${FOXCLAW_LIVE_REQUIRE_PROFILE_KEYS:-${CLAWDBOT_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
  -v "$ROOT_DIR":/src:ro \
