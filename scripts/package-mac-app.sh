...
# Build and bundle FoxClaw into a minimal .app we can open.
# Outputs to dist/FoxClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FoxClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FoxClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.foxclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/foxclaw/foxclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FoxClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FoxClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FoxClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FoxClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FoxClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FoxClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FoxClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FoxClaw/Resources/FoxClaw.icns" "$APP_ROOT/Contents/Resources/FoxClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FoxClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FoxClawKit resources"
FOXCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FoxClawKit_FoxClawKit.bundle"
if [ -d "$FOXCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FoxClawKit_FoxClawKit.bundle"
  cp -R "$FOXCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FoxClawKit_FoxClawKit.bundle"
else
  echo "WARN: FoxClawKit resource bundle not found at $FOXCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FoxClaw"
killall -q FoxClaw 2>/dev/null || true
