...
INSTALL_URL="${FOXCLAW_INSTALL_URL:-https://foxclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${FOXCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FOXCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="foxclaw"
PACKAGE_NAME="${FOXCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FOXCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FOXCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FOXCLAW_INSTALL_LATEST_OUT:-}"
fi
