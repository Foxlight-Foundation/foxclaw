# FoxClaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://foxclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://foxclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\foxclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 FoxClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one FoxClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-FoxClawNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing FoxClaw (foxclaw@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g foxclaw@$Version --no-fund --no-audit 2>&1
        Write-Host "FoxClaw installed" -Level success
        return $true
    } catch {
...
function Install-FoxClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing FoxClaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/foxclaw/foxclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\foxclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\foxclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FoxClaw from git to $GitDir" -Level info
        } else {
            Install-FoxClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FoxClaw via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-FoxClawNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'foxclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 FoxClaw installed successfully!" -Level success
}
