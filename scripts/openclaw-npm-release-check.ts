...
const EXPECTED_REPOSITORY_URL = "https://github.com/foxclaw/foxclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "foxclaw") {
    errors.push(`package.json name must be "foxclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.foxclaw !== "foxclaw.mjs") {
    errors.push(
      `package.json bin.foxclaw must be "foxclaw.mjs"; found "${pkg.bin?.foxclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`foxclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `foxclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
