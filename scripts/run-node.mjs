...
const shouldBuild = (deps) => {
  if (deps.env.FOXCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.FOXCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[foxclaw] ${message}\n`);
};

const runFoxClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["foxclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
