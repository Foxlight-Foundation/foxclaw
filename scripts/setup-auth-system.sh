#!/bin/bash
# Setup FoxClaw Auth Management System
...
echo "=== FoxClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'foxclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FoxClaw message: Send warning via FoxClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/foxclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/foxclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/foxclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now foxclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status foxclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
