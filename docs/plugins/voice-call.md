...
read_when:
  - You want to place an outbound voice call from FoxClaw
...
Voice calls for FoxClaw via a plugin. Supports outbound notifications and
...
- Use `foxclaw voicecall ...` or the `voice_call` tool
...
```bash
foxclaw plugins install ./extensions/voice-call
