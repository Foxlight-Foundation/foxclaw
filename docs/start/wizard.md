...
The onboarding wizard is the **recommended** way to set up FoxClaw on macOS,
...
`foxclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`foxclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.foxclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `foxclaw doctor` first.
</Note>
...
Use `foxclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.foxclaw/workspace-<agentId>`.
...
- CLI command reference: [`foxclaw onboard`](/cli/onboard)
