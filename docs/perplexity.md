...
FoxClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, FoxClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `foxclaw configure --section web`. It stores the key in
`~/.foxclaw/foxclaw.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.foxclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-foxclaw-load-environment-variables).
