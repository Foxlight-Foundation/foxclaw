---
summary: "Use OpenAI via API keys or Codex subscription in FoxClaw"
read_when:
  - You want to use OpenAI models in FoxClaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like FoxClaw.
...
```bash
foxclaw onboard --auth-choice openai-api-key
# or non-interactive
foxclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. FoxClaw forwards both through the `openai/*` Responses path.
FoxClaw intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

FoxClaw does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in FoxClaw.
...
foxclaw onboard --auth-choice openai-codex

# Or run OAuth directly
foxclaw models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. FoxClaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, FoxClaw also supports:

- `openai-codex/gpt-5.3-codex-spark`

FoxClaw treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

FoxClaw also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
FoxClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), FoxClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. FoxClaw enables it by default for
...
FoxClaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
FoxClaw exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, FoxClaw applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), FoxClaw now auto-enables OpenAI server-side
