---
summary: "Use Venice AI privacy-focused models in FoxClaw"
read_when:
  - You want privacy-focused inference in FoxClaw
...
```bash
foxclaw onboard --auth-choice venice-api-key
```
...
```bash
foxclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, FoxClaw shows all available Venice models. Pick based on your needs:
...
```bash
foxclaw models set venice/kimi-k2-5
foxclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
foxclaw models list | grep venice
```

## Configure via `foxclaw configure`

1. Run `foxclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FoxClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
foxclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
foxclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
foxclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
foxclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
foxclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `foxclaw models list` to see currently available models. Some models may be temporarily offline.
