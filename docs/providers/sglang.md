---
summary: "Run FoxClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run FoxClaw against a local SGLang server
...
FoxClaw can connect to SGLang using the `openai-completions` API.

FoxClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, FoxClaw will query:
