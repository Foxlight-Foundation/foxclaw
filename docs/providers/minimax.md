---
summary: "Use MiniMax M2.5 in FoxClaw"
read_when:
  - You want MiniMax models in FoxClaw
...
```bash
foxclaw plugins enable minimax-portal-auth  # skip if already loaded.
foxclaw gateway restart  # restart if gateway is already running
foxclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/foxclaw/foxclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `foxclaw.json`:
...
- Use `foxclaw models list` and `foxclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `foxclaw configure` and selecting **MiniMax M2.5**, or
