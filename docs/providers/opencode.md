---
summary: "Use OpenCode Zen and Go catalogs with FoxClaw"
read_when:
...
OpenCode exposes two hosted catalogs in FoxClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. FoxClaw keeps the runtime provider ids
...
foxclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
foxclaw onboard --auth-choice opencode-go
foxclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
