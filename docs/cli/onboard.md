---
summary: "CLI reference for `foxclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
foxclaw onboard
foxclaw onboard --flow quickstart
foxclaw onboard --flow manual
foxclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`FOXCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `foxclaw gateway run`.
...
- Fastest first chat: `foxclaw dashboard` (Control UI, no channel setup).
