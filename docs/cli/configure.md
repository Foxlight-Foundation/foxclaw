---
summary: "CLI reference for `foxclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `foxclaw config` without a subcommand opens the same wizard. Use
`foxclaw config get|set|unset` for non-interactive edits.
...
```bash
foxclaw configure
foxclaw configure --section model --section channels
```
