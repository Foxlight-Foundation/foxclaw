...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a FoxClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`foxclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the FoxClaw gateway or agent instead.                                                                                                                                      |
...
# Remote Gateway
foxclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
foxclaw acp --url wss://gateway-host:18789 --token-file ~/.foxclaw/gateway.token

# Attach to an existing session key
foxclaw acp --session agent:main:main

# Attach by label (must already exist)
foxclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
foxclaw acp --session agent:main:main --reset-session
```
...
foxclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.foxclaw/gateway.token

# Override the server command (default: foxclaw)
foxclaw acp client --server "node" --server-args foxclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a FoxClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `foxclaw acp` over stdio.

Example config (persisted):

```bash
foxclaw config set gateway.remote.url wss://gateway-host:18789
foxclaw config set gateway.remote.token <token>
```
...
```bash
foxclaw acp --session agent:main:main
foxclaw acp --session agent:design:main
foxclaw acp --session agent:qa:bug-123
```
...
FoxClaw bot over ACP, use `acpx` with its built-in `foxclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx foxclaw` at `foxclaw acp`.
3. Target the FoxClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default FoxClaw ACP session
acpx foxclaw exec "Summarize the active FoxClaw session state."

# Persistent named session for follow-up turns
acpx foxclaw sessions ensure --name codex-bridge
acpx foxclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my FoxClaw work agent for recent context relevant to this repo."
```

If you want `acpx foxclaw` to target a specific Gateway and session key every
time, override the `foxclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "foxclaw": {
      "command": "env FOXCLAW_HIDE_BANNER=1 FOXCLAW_SUPPRESS_NOTES=1 foxclaw acp --url ws://127.0.0.1:18789 --token-file ~/.foxclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local FoxClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env FOXCLAW_HIDE_BANNER=1 FOXCLAW_SUPPRESS_NOTES=1 node foxclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an FoxClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “FoxClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`FOXCLAW_GATEWAY_TOKEN`, `FOXCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`FOXCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `FOXCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `foxclaw acp client` sets `FOXCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `foxclaw`).
