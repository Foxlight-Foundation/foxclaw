...
read_when:
  ...
  - Wiring external systems into FoxClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your FoxClaw config directory (typically `~/.foxclaw/hooks/transforms`).
...
- `foxclaw webhooks gmail setup` writes `hooks.gmail` config for `foxclaw webhooks gmail run`.
