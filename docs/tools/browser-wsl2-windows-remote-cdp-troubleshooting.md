...
read_when:
  - Running FoxClaw Gateway in WSL2 while Chrome lives on Windows
...
- FoxClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/foxclaw/foxclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome-relay` profile plus the FoxClaw Chrome extension.
...
- FoxClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, FoxClaw is not the problem yet.
...
Fix that before touching FoxClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point FoxClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting FoxClaw to succeed
...
```bash
foxclaw browser open https://example.com --browser-profile remote
foxclaw browser tabs --browser-profile remote
```

For the extension relay:

```bash
foxclaw browser tabs --browser-profile chrome-relay
```

Good result:

- the tab opens in Windows Chrome
- `foxclaw browser tabs` returns the target
...
3. FoxClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug FoxClaw config or Control UI auth
