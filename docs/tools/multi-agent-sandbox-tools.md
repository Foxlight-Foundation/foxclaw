...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `foxclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `foxclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=foxclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${FOXCLAW_STATE_DIR:-$HOME/.foxclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
