...
- **gateway host** → `foxclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, FoxClaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.foxclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.foxclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `foxclaw approvals allowlist ...`).
- `foxclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `foxclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.foxclaw/exec-approvals.json` directly.

CLI: `foxclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, FoxClaw does not treat
...
topics, FoxClaw preserves the topic for the approval prompt and the post-approval follow-up.
