...
FoxClaw supports **camera capture** for agent workflows:
...
```bash
foxclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
foxclaw nodes camera snap --node <id> --facing front
foxclaw nodes camera clip --node <id> --duration 3000
foxclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`foxclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `foxclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
foxclaw nodes camera list --node <id>            # list camera ids
foxclaw nodes camera snap --node <id>            # prints MEDIA:<path>
foxclaw nodes camera snap --node <id> --max-width 1280
foxclaw nodes camera snap --node <id> --delay-ms 2000
foxclaw nodes camera snap --node <id> --device-id <id>
foxclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
foxclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
foxclaw nodes camera clip --node <id> --device-id <id>
foxclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `foxclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
foxclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
