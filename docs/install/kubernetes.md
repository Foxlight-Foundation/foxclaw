---
summary: "Deploy FoxClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run FoxClaw on a Kubernetes cluster
  - You want to test FoxClaw in a Kubernetes environment
title: "Kubernetes"
---

# FoxClaw on Kubernetes

A minimal starting point for running FoxClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

FoxClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/foxclaw 18789:18789 -n foxclaw
...
```bash
kubectl get secret foxclaw-secrets -n foxclaw -o jsonpath='{.data.FOXCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: foxclaw (configurable via FOXCLAW_NAMESPACE)
├── Deployment/foxclaw        # Single pod, init container + gateway
├── Service/foxclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/foxclaw-config  # foxclaw.json + AGENTS.md
└── Secret/foxclaw-secrets    # Gateway token + API keys
```
...
Edit `foxclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret foxclaw-secrets -n foxclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/foxclaw -n foxclaw
```

### Custom namespace

```bash
FOXCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/foxclaw/foxclaw:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # foxclaw.json + AGENTS.md
