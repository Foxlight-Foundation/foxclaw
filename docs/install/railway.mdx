...
Deploy FoxClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/foxclaw` — Control UI
...
- `FOXCLAW_STATE_DIR=/data/.foxclaw` (recommended)
- `FOXCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `FOXCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your FoxClaw state + workspace so you can migrate to another host without losing config or memory.
