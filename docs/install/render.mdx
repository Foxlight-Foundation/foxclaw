...
Deploy FoxClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/foxclaw/foxclaw)
...
repository configures everything needed to run FoxClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FOXCLAW_STATE_DIR
        value: /data/.foxclaw
      - key: FOXCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: FOXCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/foxclaw`.
...
If you use the original FoxClaw repository, Render will not auto-deploy your FoxClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For FoxClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any FoxClaw host.
