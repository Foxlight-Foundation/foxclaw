...
service via the external `foxclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.foxclaw.gateway`
  (or `ai.foxclaw.<profile>` when using `--profile`/`FOXCLAW_PROFILE`; legacy `com.foxclaw.*` is supported).
