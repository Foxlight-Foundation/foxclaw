...
- Same path: run the app from a fixed location (for FoxClaw, `dist/FoxClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.foxclaw.mac
sudo tccutil reset ScreenCapture ai.foxclaw.mac
...
Workaround: move files into the FoxClaw workspace (`~/.foxclaw/workspace`) if you want to avoid per-folder grants.
