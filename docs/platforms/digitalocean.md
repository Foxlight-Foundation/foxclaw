---
summary: "FoxClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up FoxClaw on DigitalOcean
  - Looking for cheap VPS hosting for FoxClaw
title: "DigitalOcean"
---

# FoxClaw on DigitalOcean

## Goal

Run a persistent FoxClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status foxclaw-gateway.service

# View logs
journalctl --user -u foxclaw-gateway.service -f
```
...
```bash
foxclaw config set gateway.bind tailnet
foxclaw gateway restart
```
...
```bash
foxclaw channels login whatsapp
# Scan QR code
```
...
- `~/.foxclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u foxclaw --no-pager -n 50
```
