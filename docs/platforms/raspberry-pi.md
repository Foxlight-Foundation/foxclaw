---
summary: "FoxClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FoxClaw on a Raspberry Pi
  - Running FoxClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# FoxClaw on Raspberry Pi

## Goal

Run a persistent, always-on FoxClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u foxclaw -f
```

## 9) Access the FoxClaw Dashboard
...
```bash
ssh user@gateway-host 'foxclaw dashboard --no-open'
```
...
(or `FOXCLAW_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/foxclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/foxclaw-compile-cache
mkdir -p /var/tmp/foxclaw-compile-cache
export FOXCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `FOXCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running FoxClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit foxclaw
```

```ini
[Service]
Environment=FOXCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/foxclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep FoxClaw state/cache on SSD-backed storage to avoid SD-card
...
Most FoxClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled foxclaw

# Enable if not
sudo systemctl enable foxclaw

# Start on boot
sudo systemctl start foxclaw
```
...
```bash
# Check logs
journalctl -u foxclaw --no-pager -n 100

# Common fix: rebuild
cd ~/foxclaw  # if using hackable install
npm run build
sudo systemctl restart foxclaw
```
