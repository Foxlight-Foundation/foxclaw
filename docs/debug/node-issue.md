...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FoxClaw
...
Running FoxClaw via Node with `tsx` fails at startup with:

```
[foxclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `foxclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch foxclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node foxclaw.mjs status` works on Node 25.
