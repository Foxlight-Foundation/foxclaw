...
- `/usage cost` in chats: local cost summary aggregated from FoxClaw session logs.
- CLI: `foxclaw status --usage` prints a full per-provider breakdown.
- CLI: `foxclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
