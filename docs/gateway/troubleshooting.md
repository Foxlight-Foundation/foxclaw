...
- `foxclaw gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `foxclaw doctor` reports no blocking config/service issues.
- `foxclaw channels status --probe` shows connected/ready channels.
...
foxclaw config get agents.defaults.models
```
...
foxclaw status
foxclaw channels status --probe
foxclaw pairing list --channel <channel> [--account <id>]
foxclaw config get channels
foxclaw logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `foxclaw config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `foxclaw devices list` then `foxclaw devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `foxclaw configure`). If you are running FoxClaw via Podman using the dedicated `foxclaw` user, the config lives at `~foxclaw/.foxclaw/foxclaw.json`.
...
foxclaw cron status
foxclaw cron list
foxclaw cron runs --id <jobId> --limit 20
foxclaw system heartbeat last
foxclaw logs --follow
```
...
foxclaw approvals get --node <idOrNameOrIp>
foxclaw logs --follow
foxclaw status
```
...
foxclaw browser start --browser-profile foxclaw
...
```bash
foxclaw gateway status
foxclaw config get gateway.mode
foxclaw config get gateway.remote.url
foxclaw config get gateway.auth.mode
```
...
```bash
foxclaw config get gateway.bind
