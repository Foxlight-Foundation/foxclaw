...
FoxClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `FOXCLAW_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, FoxClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). FoxClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/FoxClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/foxclaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `foxclaw.json` refs)
...
When reload-time activation fails after a healthy state, FoxClaw enters degraded secrets state.
...
- Strict command paths (for example `foxclaw memory` remote-memory paths and `foxclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `foxclaw status`, `foxclaw status --all`, `foxclaw channels status`, `foxclaw channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `foxclaw secrets reload`.
...
```bash
foxclaw secrets audit --check
foxclaw secrets configure
foxclaw secrets audit --check
```
...
- plaintext values at rest (`foxclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `foxclaw.json` refs)
...
- lets you select supported secret-bearing fields in `foxclaw.json` plus `auth-profiles.json` for one agent scope
...
- `foxclaw secrets configure --providers-only`
- `foxclaw secrets configure --skip-provider-setup`
- `foxclaw secrets configure --agent <id>`
...
```bash
foxclaw secrets apply --from /tmp/foxclaw-secrets-plan.json
foxclaw secrets apply --from /tmp/foxclaw-secrets-plan.json --dry-run
```
...
FoxClaw intentionally does not write rollback backups containing historical plaintext secret values.
