...
`foxclaw doctor` is the repair + migration tool for FoxClaw. It fixes stale
...
- Extra workspace dir detection (`~/foxclaw`).
...
you to run `foxclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.foxclaw/foxclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.foxclaw/sessions/` to `~/.foxclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.foxclaw/agent/` to `~/.foxclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.foxclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.foxclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `foxclaw doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.foxclaw/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.foxclaw` folders exist across
  home directories or when `FOXCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.foxclaw/foxclaw.json` is
...
offers to remove them and install the FoxClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named FoxClaw gateway services are considered first-class and are not
flagged as "extra."
...
- `foxclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `foxclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `foxclaw doctor` prompts before rewriting supervisor config.
- `foxclaw doctor --yes` accepts the default repair prompts.
- `foxclaw doctor --repair` applies recommended fixes without prompts.
- `foxclaw doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `foxclaw gateway install --force`.
