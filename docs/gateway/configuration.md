...
read_when:
  - Setting up FoxClaw for the first time
...
FoxClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.foxclaw/foxclaw.json`.

If the file is missing, FoxClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `foxclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    foxclaw onboard       # full setup wizard
    foxclaw configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    foxclaw config get agents.defaults.workspace
    foxclaw config set agents.defaults.heartbeat.every "2h"
    foxclaw config unset tools.web.search.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.foxclaw/foxclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
FoxClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`foxclaw doctor`, `foxclaw logs`, `foxclaw health`, `foxclaw status`)
- Run `foxclaw doctor` to see exact issues
- Run `foxclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `foxclaw.json`.
    ...
    ```bash
    foxclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `FOXCLAW_APNS_RELAY_BASE_URL` and `FOXCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `FOXCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.foxclaw/foxclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `foxclaw config set` for single keys.
    </Warning>
    ...
    foxclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.foxclaw/workspace\" } } }",
      "baseHash": "<hash>",
...
FoxClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.foxclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, FoxClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `FOXCLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${FOXCLAW_GATEWAY_TOKEN}" } },
