...
read_when:
  - Running FoxClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of FoxClaw
...
- You run FoxClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. FoxClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. FoxClaw extracts the user identity from the configured header
...
- FoxClaw can stay on loopback HTTP behind the proxy.
...
If FoxClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://foxclaw.example.com
    to: http://foxclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`foxclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update FoxClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `foxclaw security audit` and review findings
