...
- During heartbeat runs, FoxClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `foxclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FoxClaw skips the heartbeat run entirely (no model call).
...
```bash
foxclaw system event --text "Check for urgent follow-ups" --mode now
```
