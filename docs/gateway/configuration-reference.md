...
description: "Complete field-by-field reference for ~/.foxclaw/foxclaw.json"
summary: "Complete reference for every FoxClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.foxclaw/foxclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — FoxClaw uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.foxclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `foxclaw doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `foxclaw doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["foxclaw-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-foxclaw",
          requireMention: false,
...
- FoxClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `foxclaw plugins install @foxclaw/mattermost`.
...
- `commands.callbackUrl` must resolve to the FoxClaw gateway endpoint and be reachable from the Mattermost server.
...
FoxClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `foxclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, FoxClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `foxclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@foxclaw", "foxclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@foxclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `foxclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, FoxClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/foxclaw" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, FoxClaw assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.foxclaw/sandboxes",
        docker: {
          image: "foxclaw-sandbox:bookworm-slim",
          containerPrefix: "foxclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "foxclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "foxclaw-sandbox-browser:bookworm-slim",
          network: "foxclaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.foxclaw/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `foxclaw.json`.
noVNC observer access uses VNC auth by default and FoxClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `foxclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from FOXCLAW_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `FOXCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `FOXCLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `FOXCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.foxclaw/workspace",
        agentDir: "~/.foxclaw/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.foxclaw/workspace-home" },
      { id: "work", workspace: "~/.foxclaw/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, FoxClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.foxclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.foxclaw/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.foxclaw/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.foxclaw/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, FoxClaw starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.foxclaw/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, FoxClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:foxclaw`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.foxclaw/attachments/<uuid>/` with a `.manifest.json`.
...
FoxClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.foxclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `FOXCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), FoxClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `foxclaw onboard --auth-choice opencode-zen` or `foxclaw onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `foxclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `foxclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `foxclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `foxclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `foxclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.foxclaw/extensions`, `<workspace>/.foxclaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `foxclaw plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FOXCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FOXCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `FOXCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `FOXCLAW_APNS_RELAY_BASE_URL` / `FOXCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `FOXCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
FOXCLAW_CONFIG_PATH=~/.foxclaw/a.json \
FOXCLAW_STATE_DIR=~/.foxclaw-a \
foxclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.foxclaw-dev` + port `19001`), `--profile <name>` (uses `~/.foxclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.foxclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-foxclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FOXCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.foxclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or FOXCLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__foxclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__foxclaw__/a2ui/`
...
- Also serves A2UI at `/__foxclaw__/a2ui/`.
...
- Hostname defaults to `foxclaw`. Override with `FOXCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.foxclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `foxclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.foxclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${FOXCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `foxclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.foxclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/foxclaw-vault-resolver",
...
- Legacy OAuth imports from `~/.foxclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/foxclaw/foxclaw-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one FoxClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `FOXCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `foxclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `foxclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
