...
FoxClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`foxclaw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "foxclaw:<agentId>"`, `model: "agent:<agentId>"`, or `x-foxclaw-agent-id`
- use `x-foxclaw-session-key` for explicit session routing
