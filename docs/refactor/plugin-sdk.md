...
- Publish as `foxclaw/plugin-sdk` (or export from core under `foxclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `FoxClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FoxClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FoxClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FoxClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FoxClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FoxClawConfig): string;
  };
};
```
...
- Introduce `foxclaw/plugin-sdk`.
- Add `api.runtime` to `FoxClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `foxclawRuntime: ">=2026.2.0"`).
