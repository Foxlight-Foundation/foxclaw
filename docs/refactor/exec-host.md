...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.foxclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.foxclaw/node.json` (node id + pairing token).
...
Path: `~/.foxclaw/exec-approvals.json`
...
- Unix socket at `~/.foxclaw/exec-approvals.sock` (0600).
