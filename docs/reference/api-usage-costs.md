...
FoxClaw features that can generate provider usage or paid API calls.
...
- `foxclaw status --usage` and `foxclaw channels list` show provider **usage windows**
...
FoxClaw can pick up credentials from:
...
- `foxclaw status --usage`
- `foxclaw models status --json`
...
`foxclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
