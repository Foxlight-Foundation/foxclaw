---
summary: "How FoxClaw builds prompt context and reports token usage + costs"
read_when:
...
FoxClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FoxClaw assembles its own system prompt on every run. It includes:
...
For images, FoxClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FoxClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `foxclaw status --usage` and `foxclaw channels list` show
...
`cacheWrite`. If pricing is missing, FoxClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. FoxClaw can
...
Anthropic's 1M context window is currently beta-gated. FoxClaw can inject the
...
FoxClaw skips the `context-1m-*` beta header because Anthropic currently
