...
- With Anthropic API-key auth profiles, FoxClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, FoxClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
FoxClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.foxclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$FOXCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `FOXCLAW_CACHE_TRACE=1` enables cache tracing.
- `FOXCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `FOXCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `FOXCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `FOXCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
