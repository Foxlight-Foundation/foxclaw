...
FoxClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- FoxClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- FoxClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
