...
This is the full reference for the `foxclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.foxclaw/credentials/oauth.json`; auth profiles live in `~/.foxclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.foxclaw/credentials/oauth.json` (or `$FOXCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.foxclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `foxclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `foxclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `foxclaw health`.
    - Tip: `foxclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export FOXCLAW_GATEWAY_TOKEN="your-token"
foxclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env FOXCLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.foxclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.foxclaw/foxclaw.json`:
...
`foxclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.foxclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.foxclaw/agents/<agentId>/sessions/`.
