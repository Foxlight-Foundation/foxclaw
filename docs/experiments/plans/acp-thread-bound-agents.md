...
This plan defines how FoxClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- FoxClaw core owns ACP control plane concerns
...
FoxClaw should not reimplement ACP transport internals in core.
FoxClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in FoxClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under FoxClaw state dir
...
- ACP event stream is normalized to FoxClaw reply chunks
...
For the first production backend (`extensions/acpx`), FoxClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses FoxClaw `sessionKey`)
...
- FoxClaw consumes ndjson events from `acpx --format json --json-strict`
