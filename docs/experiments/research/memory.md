...
read_when:
  - Designing workspace memory (~/.foxclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FoxClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.foxclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.foxclaw/workspace` as canonical human-readable memory.
...
```
~/.foxclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in FoxClaw**, but keep a separable core library.

### Why integrate into FoxClaw?

- FoxClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `foxclaw memory recall "…" --k 25 --since 30d`
  - `foxclaw memory reflect --since 7d`
...
Pragmatic take for `~/.foxclaw/workspace`:
