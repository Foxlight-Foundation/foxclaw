...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FoxClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current FoxClaw releases, so no separate plugin install
is required.
...
```bash
foxclaw plugins install @foxclaw/feishu
```
...
If you just installed FoxClaw, run the wizard:
...
2. Configuring app credentials in FoxClaw
...
1. You already ran `foxclaw channels add` for Feishu
2. The gateway is running (`foxclaw gateway status`)
...
```bash
foxclaw pairing approve feishu <CODE>
```
...
2. Run `foxclaw logs --follow` and look for `chat_id`
...
2. Run `foxclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `foxclaw gateway status`  | Show gateway status           |
| `foxclaw gateway install` | Install/start gateway service |
| `foxclaw gateway stop`    | Stop gateway service          |
| `foxclaw gateway restart` | Restart gateway service       |
| `foxclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `foxclaw logs --follow`
...
5. Ensure the gateway is running: `foxclaw gateway status`
6. Check logs: `foxclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.foxclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.foxclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
