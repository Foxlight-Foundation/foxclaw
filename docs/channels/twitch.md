...
read_when:
  - Setting up Twitch chat integration for FoxClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FoxClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
foxclaw plugins install @foxclaw/twitch
```

Local checkout (when running from a git repo):

```bash
foxclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FOXCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "foxclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FOXCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
FOXCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
