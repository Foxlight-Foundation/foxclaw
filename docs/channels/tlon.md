...
Tlon is a decentralized messenger built on Urbit. FoxClaw connects to your Urbit ship and can
...
```bash
foxclaw plugins install @foxclaw/tlon
```

Local checkout (when running from a git repo):

```bash
foxclaw plugins install ./extensions/tlon
```
...
By default, FoxClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `foxclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FoxClaw replies in-thread.
