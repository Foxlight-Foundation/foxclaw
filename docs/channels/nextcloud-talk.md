...
```bash
foxclaw plugins install @foxclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
foxclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FoxClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `foxclaw pairing list nextcloud-talk`
  - `foxclaw pairing approve nextcloud-talk <CODE>`
