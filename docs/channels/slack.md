...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure FoxClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `foxclaw pairing approve slack <code>`.
...
- The interactive callback values are FoxClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, FoxClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while FoxClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "FoxClaw",
    "description": "Slack connector for FoxClaw"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/foxclaw",
        "description": "Send a message to FoxClaw",
        "should_escape": false
      }
    ]
  },
...
FoxClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, FoxClaw falls back to normal delivery for remaining payloads.
