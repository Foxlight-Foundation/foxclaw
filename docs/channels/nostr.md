...
read_when:
  - You want FoxClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables FoxClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`foxclaw onboard`) and `foxclaw channels add` list optional channel plugins.
...
```bash
foxclaw plugins install @foxclaw/nostr
```

Use a local checkout (dev workflows):

```bash
foxclaw plugins install --link <path-to-foxclaw>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "foxclaw@example.com",
        "lud16": "foxclaw@example.com"
      }
    }
  }
}
