...
Matrix is an open, decentralized messaging protocol. FoxClaw connects as a Matrix **user**
...
```bash
foxclaw plugins install @foxclaw/matrix
```

Local checkout (when running from a git repo):

```bash
foxclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `foxclaw plugins install @foxclaw/matrix`
   - From a local checkout: `foxclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FoxClaw calls the same
     login endpoint, stores the access token in `~/.foxclaw/credentials/matrix/credentials.json`,
...
- On first connection, FoxClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FoxClaw logs a warning.
...
`~/.foxclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `foxclaw pairing list matrix`
  - `foxclaw pairing approve matrix <CODE>`
...
- On startup, FoxClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
