...
FoxClaw routes replies **back to the channel where a message came from**. The
...
FoxClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, FoxClaw still records inbound session metadata, but it
...
Session stores live under the state directory (default `~/.foxclaw`):

- `~/.foxclaw/agents/<agentId>/sessions/sessions.json`
