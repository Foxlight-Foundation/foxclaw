...
read_when:
  - Setting up Zalo Personal for FoxClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside FoxClaw.
...
- Install via CLI: `foxclaw plugins install @foxclaw/zalouser`
- Or from a source checkout: `foxclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `foxclaw channels login --channel zalouser`
...
```bash
foxclaw directory self --channel zalouser
foxclaw directory peers list --channel zalouser --query "name"
foxclaw directory groups list --channel zalouser --query "work"
```
...
- `foxclaw pairing list zalouser`
- `foxclaw pairing approve zalouser <code>`
...
- On startup, FoxClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, FoxClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in FoxClaw state. Example:
...
- FoxClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, FoxClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `foxclaw channels logout --channel zalouser && foxclaw channels login --channel zalouser`
...
- The channel now runs fully in FoxClaw without external CLI binaries.
