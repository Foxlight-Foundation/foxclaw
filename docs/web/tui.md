...
```bash
foxclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `foxclaw tui --deliver`
...
- Local shell commands receive `FOXCLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `FOXCLAW_THEME=light` before launching `foxclaw tui`.
- To force the original dark palette instead, set `FOXCLAW_THEME=dark`.
...
- Check the Gateway logs: `foxclaw logs --follow`.
- Confirm the agent can run: `foxclaw status` and `foxclaw models status`.
...
- No agents in picker: check `foxclaw agents list` and your routing config.
