...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/foxclaw`)
...
If the page fails to load, start the Gateway first: `foxclaw gateway`.
...
you revoke it with `foxclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.foxclaw/foxclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop foxclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FoxClaw
...
FoxClaw **blocks** Control UI connections without device identity.
