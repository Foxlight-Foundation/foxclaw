...
- `~/Library/Application Support/FoxClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `foxclaw-canvas://<session>/<path>`

示例：

- `foxclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `foxclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `foxclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
foxclaw nodes canvas present --node <id>
foxclaw nodes canvas navigate --node <id> --url "/"
foxclaw nodes canvas eval --node <id> --js "document.title"
foxclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__foxclaw__/a2ui/
```
...
foxclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
foxclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "foxclaw://agent?message=Review%20this%20design";
```
