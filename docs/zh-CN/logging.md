...
你可以在 `~/.foxclaw/foxclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.foxclaw/foxclaw.json` 的 `logging` 下。
...
- FoxClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `foxclaw plugins enable diagnostics-otel` 启用插件。
...
- `foxclaw.tokens`（计数器，属性：`foxclaw.token`、`foxclaw.channel`、`foxclaw.provider`、`foxclaw.model`）
- `foxclaw.cost.usd`（计数器，属性：`foxclaw.channel`、`foxclaw.provider`、`foxclaw.model`）
- `foxclaw.run.duration_ms`（直方图，属性：`foxclaw.channel`、`foxclaw.provider`、`foxclaw.model`）
- `foxclaw.context.tokens`（直方图，属性：`foxclaw.context`、`foxclaw.channel`、`foxclaw.provider`、`foxclaw.model`）

消息流：

- `foxclaw.webhook.received`（计数器，属性：`foxclaw.channel`、`foxclaw.webhook`）
- `foxclaw.webhook.error`（计数器，属性：`foxclaw.channel`、`foxclaw.webhook`）
- `foxclaw.webhook.duration_ms`（直方图，属性：`foxclaw.channel`、`foxclaw.webhook`）
- `foxclaw.message.queued`（计数器，属性：`foxclaw.channel`、`foxclaw.source`）
- `foxclaw.message.processed`（计数器，属性：`foxclaw.channel`、`foxclaw.outcome`）
- `foxclaw.message.duration_ms`（直方图，属性：`foxclaw.channel`、`foxclaw.outcome`）

队列 + 会话：

- `foxclaw.queue.lane.enqueue`（计数器，属性：`foxclaw.lane`）
- `foxclaw.queue.lane.dequeue`（计数器，属性：`foxclaw.lane`）
- `foxclaw.queue.depth`（直方图，属性：`foxclaw.lane` 或 `foxclaw.channel=heartbeat`）
- `foxclaw.queue.wait_ms`（直方图，属性：`foxclaw.lane`）
- `foxclaw.session.state`（计数器，属性：`foxclaw.state`、`foxclaw.reason`）
- `foxclaw.session.stuck`（计数器，属性：`foxclaw.state`）
- `foxclaw.session.stuck_age_ms`（直方图，属性：`foxclaw.state`）
- `foxclaw.run.attempt`（计数器，属性：`foxclaw.attempt`）
...
- `foxclaw.webhook.error`
  - `foxclaw.channel`、`foxclaw.webhook`、`foxclaw.chatId`、`foxclaw.error`
- `foxclaw.message.processed`
  - `foxclaw.channel`、`foxclaw.outcome`、`foxclaw.chatId`、`foxclaw.messageId`、`foxclaw.sessionKey`、`foxclaw.sessionId`、`foxclaw.reason`
- `foxclaw.session.stuck`
  - `foxclaw.state`、`foxclaw.ageMs`、`foxclaw.queueDepth`、`foxclaw.sessionKey`、`foxclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `foxclaw doctor`。
