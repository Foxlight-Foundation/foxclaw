...
metadata: { "foxclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == FoxClaw `sessions_spawn`
- **File I/O** == FoxClaw `read`/`write`
- **Remote fetch** == FoxClaw `web_fetch` (or `exec` with curl when POST is required)
