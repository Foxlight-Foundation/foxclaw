...
vi.mock("foxclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("foxclaw/plugin-sdk/diagnostics-otel")>(
    "foxclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { FoxClawPluginServiceContext } from "foxclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "foxclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/foxclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): FoxClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("foxclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("foxclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("foxclaw.webhook.processed");
    expect(spanNames).toContain("foxclaw.message.processed");
    expect(spanNames).toContain("foxclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["foxclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("foxclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "foxclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["foxclaw.reason"]).toBe("string");
    expect(String(attrs?.["foxclaw.reason"])).not.toContain(
