...
import type { FoxClawPluginApi, FoxClawPluginToolContext } from "openclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<FoxClawPluginApi> = {}): FoxClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<FoxClawPluginToolContext> = {}): FoxClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: FoxClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
