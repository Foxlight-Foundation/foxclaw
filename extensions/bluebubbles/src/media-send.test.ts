...
import type { FoxClawConfig, PluginRuntime } from "foxclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): FoxClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
