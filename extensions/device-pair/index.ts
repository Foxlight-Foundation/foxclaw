...
import type { FoxClawPluginApi } from "foxclaw/plugin-sdk/device-pair";
import {
...
} from "foxclaw/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: FoxClawPluginApi["config"]): number {
  const envPort =
...
function resolveAuthLabel(cfg: FoxClawPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: FoxClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the FoxClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the FoxClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
