...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["foxclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["foxclaw", "pollId"]) ??
    readNestedString(value, ["foxclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "foxclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "foxclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          foxclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "foxclaw poll vote",
          displayText: "Vote recorded",
          value: { foxclawPollId: pollId, pollId },
        },
      },
    ],
  };
