...
import type { FoxClawPluginApi, FoxClawPluginService } from "foxclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: FoxClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FoxClawPluginApi["config"]): string[] {
...
export default function register(api: FoxClawPluginApi) {
  ...
  const timerService: FoxClawPluginService = {
