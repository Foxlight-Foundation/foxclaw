...
Read-only diff viewer plugin for **FoxClaw** agents.
...
Set plugin-wide defaults in `~/.foxclaw/foxclaw.json`:
...
Before:
FoxClaw supports plugins.

After:
FoxClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/foxclaw-diffs`).
