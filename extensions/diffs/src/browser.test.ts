...
import type { FoxClawConfig } from "foxclaw/plugin-sdk/diffs";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("foxclaw-diffs-browser-"));
...
function createConfig(): FoxClawConfig {
  return {
    browser: {
