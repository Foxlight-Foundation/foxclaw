  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/foxclaw",
      },
    },
    session: { store: "/tmp/foxclaw-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bfoxclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "foxclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "FoxClaw" },
          },
        },
        eventPatch: {
