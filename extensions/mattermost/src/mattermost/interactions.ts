...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__foxclaw_channel_id";
...
type InteractionCallbackConfig = Pick<FoxClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "foxclaw-mattermost-interactions").update(botToken).digest("hex");
}
