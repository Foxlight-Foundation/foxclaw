...
import type { FoxClawConfig } from "foxclaw/plugin-sdk/mattermost";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    const previousStateDir = process.env.FOXCLAW_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxclaw-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies FoxClawConfig;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.FOXCLAW_STATE_DIR = previousStateDir;
      }
