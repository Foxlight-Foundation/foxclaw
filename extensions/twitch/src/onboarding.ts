...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: FOXCLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: FoxClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var FOXCLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.FOXCLAW_TWITCH_ACCESS_TOKEN?.trim();
