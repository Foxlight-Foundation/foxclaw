/**
 * Twitch channel plugin for FoxClaw.
 *
...
import { buildChannelConfigSchema } from "foxclaw/plugin-sdk/twitch";
 ...
 * for FoxClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: FoxClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: FoxClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: FoxClawConfig): boolean => {
