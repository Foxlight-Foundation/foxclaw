# Google Gemini CLI Auth (FoxClaw plugin)
...
```bash
foxclaw plugins enable google-gemini-cli-auth
```
...
```bash
foxclaw models auth login --provider google-gemini-cli --set-default
```
...
- `FOXCLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `FOXCLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
