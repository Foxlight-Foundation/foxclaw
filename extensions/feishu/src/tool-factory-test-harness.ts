import type { AnyAgentTool, FoxClawPluginApi } from "foxclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: FoxClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<FoxClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
