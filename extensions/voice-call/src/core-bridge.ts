...
function resolveFoxClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.FOXCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set FOXCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveFoxClawRoot(), "dist", "extensionAPI.js");
