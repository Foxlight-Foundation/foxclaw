...
`foxclaw.invoke --each`) without adding new FoxClaw code per workflow.
...
This extension depends on FoxClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FoxClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.foxclaw/extensions` as a standalone plugin directory.
