---
name: node-connect
description: Diagnose FoxClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify FoxClaw is advertising that route, then fix pairing/auth.
...
- whether `foxclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `foxclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
foxclaw config get gateway.mode
foxclaw config get gateway.bind
foxclaw config get gateway.tailscale.mode
foxclaw config get gateway.remote.url
foxclaw config get gateway.auth.mode
foxclaw config get gateway.auth.allowTailscale
foxclaw config get plugins.entries.device-pair.config.publicUrl
foxclaw qr --json
foxclaw devices list
foxclaw nodes status
```

If this FoxClaw instance is pointed at a remote gateway, also run:

```bash
foxclaw qr --remote --json
```
...
If `foxclaw qr --json` says `Gateway is only bound to loopback`:
...
- `foxclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run foxclaw qr again, rescan, then approve the pending device pairing.`
