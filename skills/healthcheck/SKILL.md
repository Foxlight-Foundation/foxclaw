---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FoxClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FoxClaw cron scheduling for periodic checks, or version status checks on a machine running FoxClaw (laptop, workstation, Pi, VPS).
---

# FoxClaw Host Hardening

## Overview

Assess and harden the host running FoxClaw, then align it to a user-defined risk tolerance without breaking access. Use FoxClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FoxClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. FoxClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FoxClaw can access sensitive data.
...
### 2) Run FoxClaw security audits (read-only)

As part of the default read-only checks, run `foxclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `foxclaw security audit` (faster, non-probing)
2. `foxclaw security audit --json` (structured output)

Offer to apply FoxClaw safe defaults (numbered):

1. `foxclaw security audit --fix`

Be explicit that `--fix` only tightens FoxClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FoxClaw version/update status (read-only)

As part of the default read-only checks, run `foxclaw update status`.
...
- Credential hygiene notes (location of FoxClaw creds, prefer disk encryption)
...
After FoxClaw install or first hardening pass, run at least one baseline audit and version check:

- `foxclaw security audit`
- `foxclaw security audit --deep`
- `foxclaw update status`

Ongoing monitoring is recommended. Use the FoxClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `foxclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `foxclaw update status`
...
Before creating, `foxclaw cron list` and match on exact `name`. If found, `foxclaw cron edit <id> ...`.
If not found, `foxclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `foxclaw update status` (preferred for source checkouts and channels)
2. `npm view foxclaw version` (published npm version)

## FoxClaw command accuracy

Use only supported commands and flags:

- `foxclaw security audit [--deep] [--fix] [--json]`
- `foxclaw status` / `foxclaw status --deep`
- `foxclaw health --json`
- `foxclaw update status`
- `foxclaw cron add|list|runs|run`

Do not invent CLI flags or imply FoxClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FoxClaw compaction:
