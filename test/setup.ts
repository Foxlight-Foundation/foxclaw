...
process.env.FOXCLAW_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: FoxClawConfig) => {
    ...
    resolveAccount: (cfg: FoxClawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: FoxClawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
