...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `foxclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".foxclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          FOXCLAW_SKIP_PROVIDERS: "1",
          FOXCLAW_SKIP_GMAIL_WATCHER: "1",
          FOXCLAW_SKIP_CRON: "1",
          FOXCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          FOXCLAW_SKIP_CANVAS_HOST: "1",
          FOXCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
